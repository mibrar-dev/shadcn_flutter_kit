// ignore_for_file: duplicate_import, unnecessary_import, unused_import, unnecessary_null_comparison, dead_code, deprecated_member_use, use_null_aware_elements, sort_child_properties_last

import 'package:flutter/widgets.dart';

/// A collection of Lucide icons as Flutter IconData constants.
///
/// Lucide is an open-source icon library that provides a wide range of
/// carefully crafted icons for use in Flutter applications.
///
/// Each icon is available as a static constant `IconData` that can be used
/// directly with Flutter's `Icon` widget.
///
/// Example:
/// ```dart
/// Icon(
///   LucideIcons.heart,
///   size: 24,
///   color: Colors.red,
/// )
/// ```
class LucideIcons {
  LucideIcons._();

  /// Lucide icon for 'A arrow down'.
  static const IconData aArrowDown = IconData(58761, fontFamily: 'LucideIcons');

  /// Lucide icon for 'A arrow up'.
  static const IconData aArrowUp = IconData(58762, fontFamily: 'LucideIcons');

  /// Lucide icon for 'A large small'.
  static const IconData aLargeSmall = IconData(
    58763,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'accessibility'.
  static const IconData accessibility = IconData(
    58006,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'activity'.
  static const IconData activity = IconData(57400, fontFamily: 'LucideIcons');

  /// Lucide icon for 'air vent'.
  static const IconData airVent = IconData(58192, fontFamily: 'LucideIcons');

  /// Lucide icon for 'airplay'.
  static const IconData airplay = IconData(57401, fontFamily: 'LucideIcons');

  /// Lucide icon for 'alarm clock check'.
  static const IconData alarmClockCheck = IconData(
    57835,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'alarm clock minus'.
  static const IconData alarmClockMinus = IconData(
    57836,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'alarm clock off'.
  static const IconData alarmClockOff = IconData(
    57914,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'alarm clock plus'.
  static const IconData alarmClockPlus = IconData(
    57837,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'alarm clock'.
  static const IconData alarmClock = IconData(57402, fontFamily: 'LucideIcons');

  /// Lucide icon for 'alarm smoke'.
  static const IconData alarmSmoke = IconData(58751, fontFamily: 'LucideIcons');

  /// Lucide icon for 'album'.
  static const IconData album = IconData(57403, fontFamily: 'LucideIcons');

  /// Lucide icon for 'align center horizontal'.
  static const IconData alignCenterHorizontal = IconData(
    57963,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align center vertical'.
  static const IconData alignCenterVertical = IconData(
    57964,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align center'.
  static const IconData alignCenter = IconData(
    57404,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align end horizontal'.
  static const IconData alignEndHorizontal = IconData(
    57965,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align end vertical'.
  static const IconData alignEndVertical = IconData(
    57966,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align horizontal distribute center'.
  static const IconData alignHorizontalDistributeCenter = IconData(
    57405,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align horizontal distribute end'.
  static const IconData alignHorizontalDistributeEnd = IconData(
    57406,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align horizontal distribute start'.
  static const IconData alignHorizontalDistributeStart = IconData(
    57407,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align horizontal justify center'.
  static const IconData alignHorizontalJustifyCenter = IconData(
    57969,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align horizontal justify end'.
  static const IconData alignHorizontalJustifyEnd = IconData(
    57970,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align horizontal justify start'.
  static const IconData alignHorizontalJustifyStart = IconData(
    57971,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align horizontal space around'.
  static const IconData alignHorizontalSpaceAround = IconData(
    57972,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align horizontal space between'.
  static const IconData alignHorizontalSpaceBetween = IconData(
    57973,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align justify'.
  static const IconData alignJustify = IconData(
    57408,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align left'.
  static const IconData alignLeft = IconData(57409, fontFamily: 'LucideIcons');

  /// Lucide icon for 'align right'.
  static const IconData alignRight = IconData(57410, fontFamily: 'LucideIcons');

  /// Lucide icon for 'align start horizontal'.
  static const IconData alignStartHorizontal = IconData(
    57967,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align start vertical'.
  static const IconData alignStartVertical = IconData(
    57968,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align vertical distribute center'.
  static const IconData alignVerticalDistributeCenter = IconData(
    57981,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align vertical distribute end'.
  static const IconData alignVerticalDistributeEnd = IconData(
    57982,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align vertical distribute start'.
  static const IconData alignVerticalDistributeStart = IconData(
    57983,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align vertical justify center'.
  static const IconData alignVerticalJustifyCenter = IconData(
    57974,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align vertical justify end'.
  static const IconData alignVerticalJustifyEnd = IconData(
    57975,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align vertical justify start'.
  static const IconData alignVerticalJustifyStart = IconData(
    57976,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align vertical space around'.
  static const IconData alignVerticalSpaceAround = IconData(
    57977,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'align vertical space between'.
  static const IconData alignVerticalSpaceBetween = IconData(
    57978,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ambulance'.
  static const IconData ambulance = IconData(58815, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ampersand'.
  static const IconData ampersand = IconData(58528, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ampersands'.
  static const IconData ampersands = IconData(58529, fontFamily: 'LucideIcons');

  /// Lucide icon for 'amphora'.
  static const IconData amphora = IconData(58911, fontFamily: 'LucideIcons');

  /// Lucide icon for 'anchor'.
  static const IconData anchor = IconData(57411, fontFamily: 'LucideIcons');

  /// Lucide icon for 'angry'.
  static const IconData angry = IconData(58107, fontFamily: 'LucideIcons');

  /// Lucide icon for 'annoyed'.
  static const IconData annoyed = IconData(58108, fontFamily: 'LucideIcons');

  /// Lucide icon for 'antenna'.
  static const IconData antenna = IconData(58598, fontFamily: 'LucideIcons');

  /// Lucide icon for 'anvil'.
  static const IconData anvil = IconData(58756, fontFamily: 'LucideIcons');

  /// Lucide icon for 'aperture'.
  static const IconData aperture = IconData(57412, fontFamily: 'LucideIcons');

  /// Lucide icon for 'app window mac'.
  static const IconData appWindowMac = IconData(
    58838,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'app window'.
  static const IconData appWindow = IconData(58410, fontFamily: 'LucideIcons');

  /// Lucide icon for 'apple'.
  static const IconData apple = IconData(58193, fontFamily: 'LucideIcons');

  /// Lucide icon for 'archive restore'.
  static const IconData archiveRestore = IconData(
    58060,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'archive X'.
  static const IconData archiveX = IconData(58640, fontFamily: 'LucideIcons');

  /// Lucide icon for 'archive'.
  static const IconData archive = IconData(57413, fontFamily: 'LucideIcons');

  /// Lucide icon for 'armchair'.
  static const IconData armchair = IconData(58047, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow big down dash'.
  static const IconData arrowBigDownDash = IconData(
    58401,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow big down'.
  static const IconData arrowBigDown = IconData(
    57824,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow big left dash'.
  static const IconData arrowBigLeftDash = IconData(
    58402,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow big left'.
  static const IconData arrowBigLeft = IconData(
    57825,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow big right dash'.
  static const IconData arrowBigRightDash = IconData(
    58403,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow big right'.
  static const IconData arrowBigRight = IconData(
    57826,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow big up dash'.
  static const IconData arrowBigUpDash = IconData(
    58404,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow big up'.
  static const IconData arrowBigUp = IconData(57827, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow down 01'.
  static const IconData arrowDown01 = IconData(
    58391,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down 10'.
  static const IconData arrowDown10 = IconData(
    58392,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down A Z'.
  static const IconData arrowDownAZ = IconData(
    58393,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down from line'.
  static const IconData arrowDownFromLine = IconData(
    58456,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down left'.
  static const IconData arrowDownLeft = IconData(
    57415,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down narrow wide'.
  static const IconData arrowDownNarrowWide = IconData(
    57416,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down right'.
  static const IconData arrowDownRight = IconData(
    57417,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down to dot'.
  static const IconData arrowDownToDot = IconData(
    58449,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down to line'.
  static const IconData arrowDownToLine = IconData(
    58457,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down up'.
  static const IconData arrowDownUp = IconData(
    57418,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down wide narrow'.
  static const IconData arrowDownWideNarrow = IconData(
    57419,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down Z A'.
  static const IconData arrowDownZA = IconData(
    58394,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow down'.
  static const IconData arrowDown = IconData(57414, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow left from line'.
  static const IconData arrowLeftFromLine = IconData(
    58458,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow left right'.
  static const IconData arrowLeftRight = IconData(
    57929,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow left to line'.
  static const IconData arrowLeftToLine = IconData(
    58459,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow left'.
  static const IconData arrowLeft = IconData(57420, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow right from line'.
  static const IconData arrowRightFromLine = IconData(
    58460,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow right left'.
  static const IconData arrowRightLeft = IconData(
    58395,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow right to line'.
  static const IconData arrowRightToLine = IconData(
    58461,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow right'.
  static const IconData arrowRight = IconData(57421, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow up 01'.
  static const IconData arrowUp01 = IconData(58396, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow up 10'.
  static const IconData arrowUp10 = IconData(58397, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow up A Z'.
  static const IconData arrowUpAZ = IconData(58398, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow up down'.
  static const IconData arrowUpDown = IconData(
    58240,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow up from dot'.
  static const IconData arrowUpFromDot = IconData(
    58450,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow up from line'.
  static const IconData arrowUpFromLine = IconData(
    58462,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow up left'.
  static const IconData arrowUpLeft = IconData(
    57423,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow up narrow wide'.
  static const IconData arrowUpNarrowWide = IconData(
    57424,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow up right'.
  static const IconData arrowUpRight = IconData(
    57425,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow up to line'.
  static const IconData arrowUpToLine = IconData(
    58463,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow up wide narrow'.
  static const IconData arrowUpWideNarrow = IconData(
    58399,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'arrow up Z A'.
  static const IconData arrowUpZA = IconData(58400, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrow up'.
  static const IconData arrowUp = IconData(57422, fontFamily: 'LucideIcons');

  /// Lucide icon for 'arrows up from line'.
  static const IconData arrowsUpFromLine = IconData(
    58584,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'asterisk'.
  static const IconData asterisk = IconData(57838, fontFamily: 'LucideIcons');

  /// Lucide icon for 'at sign'.
  static const IconData atSign = IconData(57426, fontFamily: 'LucideIcons');

  /// Lucide icon for 'atom'.
  static const IconData atom = IconData(58330, fontFamily: 'LucideIcons');

  /// Lucide icon for 'audio lines'.
  static const IconData audioLines = IconData(58718, fontFamily: 'LucideIcons');

  /// Lucide icon for 'audio waveform'.
  static const IconData audioWaveform = IconData(
    58719,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'award'.
  static const IconData award = IconData(57427, fontFamily: 'LucideIcons');

  /// Lucide icon for 'axe'.
  static const IconData axe = IconData(57428, fontFamily: 'LucideIcons');

  /// Lucide icon for 'axis 3d'.
  static const IconData axis3d = IconData(58109, fontFamily: 'LucideIcons');

  /// Lucide icon for 'baby'.
  static const IconData baby = IconData(58061, fontFamily: 'LucideIcons');

  /// Lucide icon for 'backpack'.
  static const IconData backpack = IconData(58055, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge alert'.
  static const IconData badgeAlert = IconData(58489, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge cent'.
  static const IconData badgeCent = IconData(58643, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge check'.
  static const IconData badgeCheck = IconData(57920, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge dollar sign'.
  static const IconData badgeDollarSign = IconData(
    58490,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'badge euro'.
  static const IconData badgeEuro = IconData(58644, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge help'.
  static const IconData badgeHelp = IconData(58491, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge indian rupee'.
  static const IconData badgeIndianRupee = IconData(
    58645,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'badge info'.
  static const IconData badgeInfo = IconData(58492, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge japanese yen'.
  static const IconData badgeJapaneseYen = IconData(
    58646,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'badge minus'.
  static const IconData badgeMinus = IconData(58493, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge percent'.
  static const IconData badgePercent = IconData(
    58494,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'badge plus'.
  static const IconData badgePlus = IconData(58495, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge pound sterling'.
  static const IconData badgePoundSterling = IconData(
    58647,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'badge russian ruble'.
  static const IconData badgeRussianRuble = IconData(
    58648,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'badge swiss franc'.
  static const IconData badgeSwissFranc = IconData(
    58649,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'badge X'.
  static const IconData badgeX = IconData(58496, fontFamily: 'LucideIcons');

  /// Lucide icon for 'badge'.
  static const IconData badge = IconData(58488, fontFamily: 'LucideIcons');

  /// Lucide icon for 'baggage claim'.
  static const IconData baggageClaim = IconData(
    58056,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ban'.
  static const IconData ban = IconData(57429, fontFamily: 'LucideIcons');

  /// Lucide icon for 'banana'.
  static const IconData banana = IconData(58194, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bandage'.
  static const IconData bandage = IconData(58913, fontFamily: 'LucideIcons');

  /// Lucide icon for 'banknote'.
  static const IconData banknote = IconData(57430, fontFamily: 'LucideIcons');

  /// Lucide icon for 'barcode'.
  static const IconData barcode = IconData(58679, fontFamily: 'LucideIcons');

  /// Lucide icon for 'baseline'.
  static const IconData baseline = IconData(57988, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bath'.
  static const IconData bath = IconData(58026, fontFamily: 'LucideIcons');

  /// Lucide icon for 'battery charging'.
  static const IconData batteryCharging = IconData(
    57432,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'battery full'.
  static const IconData batteryFull = IconData(
    57433,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'battery low'.
  static const IconData batteryLow = IconData(57434, fontFamily: 'LucideIcons');

  /// Lucide icon for 'battery medium'.
  static const IconData batteryMedium = IconData(
    57435,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'battery plus'.
  static const IconData batteryPlus = IconData(
    58946,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'battery warning'.
  static const IconData batteryWarning = IconData(
    58287,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'battery'.
  static const IconData battery = IconData(57431, fontFamily: 'LucideIcons');

  /// Lucide icon for 'beaker'.
  static const IconData beaker = IconData(57436, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bean off'.
  static const IconData beanOff = IconData(58259, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bean'.
  static const IconData bean = IconData(58258, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bed double'.
  static const IconData bedDouble = IconData(58049, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bed single'.
  static const IconData bedSingle = IconData(58050, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bed'.
  static const IconData bed = IconData(58048, fontFamily: 'LucideIcons');

  /// Lucide icon for 'beef'.
  static const IconData beef = IconData(58280, fontFamily: 'LucideIcons');

  /// Lucide icon for 'beer off'.
  static const IconData beerOff = IconData(58845, fontFamily: 'LucideIcons');

  /// Lucide icon for 'beer'.
  static const IconData beer = IconData(58062, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bell dot'.
  static const IconData bellDot = IconData(58415, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bell electric'.
  static const IconData bellElectric = IconData(
    58752,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bell minus'.
  static const IconData bellMinus = IconData(57839, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bell off'.
  static const IconData bellOff = IconData(57438, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bell plus'.
  static const IconData bellPlus = IconData(57840, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bell ring'.
  static const IconData bellRing = IconData(57891, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bell'.
  static const IconData bell = IconData(57437, fontFamily: 'LucideIcons');

  /// Lucide icon for 'between horizontal end'.
  static const IconData betweenHorizontalEnd = IconData(
    58773,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'between horizontal start'.
  static const IconData betweenHorizontalStart = IconData(
    58774,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'between vertical end'.
  static const IconData betweenVerticalEnd = IconData(
    58775,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'between vertical start'.
  static const IconData betweenVerticalStart = IconData(
    58776,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'biceps flexed'.
  static const IconData bicepsFlexed = IconData(
    58863,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bike'.
  static const IconData bike = IconData(57809, fontFamily: 'LucideIcons');

  /// Lucide icon for 'binary'.
  static const IconData binary = IconData(57841, fontFamily: 'LucideIcons');

  /// Lucide icon for 'binoculars'.
  static const IconData binoculars = IconData(58917, fontFamily: 'LucideIcons');

  /// Lucide icon for 'biohazard'.
  static const IconData biohazard = IconData(58437, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bird'.
  static const IconData bird = IconData(58312, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bitcoin'.
  static const IconData bitcoin = IconData(57439, fontFamily: 'LucideIcons');

  /// Lucide icon for 'blend'.
  static const IconData blend = IconData(58784, fontFamily: 'LucideIcons');

  /// Lucide icon for 'blinds'.
  static const IconData blinds = IconData(58307, fontFamily: 'LucideIcons');

  /// Lucide icon for 'blocks'.
  static const IconData blocks = IconData(58622, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bluetooth connected'.
  static const IconData bluetoothConnected = IconData(
    57783,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bluetooth off'.
  static const IconData bluetoothOff = IconData(
    57784,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bluetooth searching'.
  static const IconData bluetoothSearching = IconData(
    57785,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bluetooth'.
  static const IconData bluetooth = IconData(57440, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bold'.
  static const IconData bold = IconData(57441, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bolt'.
  static const IconData bolt = IconData(58768, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bomb'.
  static const IconData bomb = IconData(58110, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bone'.
  static const IconData bone = IconData(58203, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book A'.
  static const IconData bookA = IconData(58696, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book audio'.
  static const IconData bookAudio = IconData(58697, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book check'.
  static const IconData bookCheck = IconData(58698, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book copy'.
  static const IconData bookCopy = IconData(58352, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book dashed'.
  static const IconData bookDashed = IconData(58353, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book down'.
  static const IconData bookDown = IconData(58354, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book headphones'.
  static const IconData bookHeadphones = IconData(
    58699,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'book heart'.
  static const IconData bookHeart = IconData(58700, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book image'.
  static const IconData bookImage = IconData(58701, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book key'.
  static const IconData bookKey = IconData(58355, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book lock'.
  static const IconData bookLock = IconData(58356, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book marked'.
  static const IconData bookMarked = IconData(58357, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book minus'.
  static const IconData bookMinus = IconData(58358, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book open check'.
  static const IconData bookOpenCheck = IconData(
    58244,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'book open text'.
  static const IconData bookOpenText = IconData(
    58702,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'book open'.
  static const IconData bookOpen = IconData(57443, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book plus'.
  static const IconData bookPlus = IconData(58359, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book text'.
  static const IconData bookText = IconData(58703, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book type'.
  static const IconData bookType = IconData(58704, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book up 2'.
  static const IconData bookUp2 = IconData(58538, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book up'.
  static const IconData bookUp = IconData(58360, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book user'.
  static const IconData bookUser = IconData(58705, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book X'.
  static const IconData bookX = IconData(58361, fontFamily: 'LucideIcons');

  /// Lucide icon for 'book'.
  static const IconData book = IconData(57442, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bookmark check'.
  static const IconData bookmarkCheck = IconData(
    58659,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bookmark minus'.
  static const IconData bookmarkMinus = IconData(
    57915,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bookmark plus'.
  static const IconData bookmarkPlus = IconData(
    57916,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bookmark X'.
  static const IconData bookmarkX = IconData(58660, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bookmark'.
  static const IconData bookmark = IconData(57444, fontFamily: 'LucideIcons');

  /// Lucide icon for 'boom box'.
  static const IconData boomBox = IconData(58610, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bot message square'.
  static const IconData botMessageSquare = IconData(
    58834,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'bot off'.
  static const IconData botOff = IconData(58852, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bot'.
  static const IconData bot = IconData(57786, fontFamily: 'LucideIcons');

  /// Lucide icon for 'box'.
  static const IconData box = IconData(57445, fontFamily: 'LucideIcons');

  /// Lucide icon for 'boxes'.
  static const IconData boxes = IconData(58063, fontFamily: 'LucideIcons');

  /// Lucide icon for 'braces'.
  static const IconData braces = IconData(58221, fontFamily: 'LucideIcons');

  /// Lucide icon for 'brackets'.
  static const IconData brackets = IconData(58439, fontFamily: 'LucideIcons');

  /// Lucide icon for 'brain circuit'.
  static const IconData brainCircuit = IconData(
    58314,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'brain cog'.
  static const IconData brainCog = IconData(58315, fontFamily: 'LucideIcons');

  /// Lucide icon for 'brain'.
  static const IconData brain = IconData(58313, fontFamily: 'LucideIcons');

  /// Lucide icon for 'brick wall'.
  static const IconData brickWall = IconData(58757, fontFamily: 'LucideIcons');

  /// Lucide icon for 'briefcase business'.
  static const IconData briefcaseBusiness = IconData(
    58841,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'briefcase conveyor belt'.
  static const IconData briefcaseConveyorBelt = IconData(
    58927,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'briefcase medical'.
  static const IconData briefcaseMedical = IconData(
    58842,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'briefcase'.
  static const IconData briefcase = IconData(57446, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bring to front'.
  static const IconData bringToFront = IconData(
    58611,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'brush'.
  static const IconData brush = IconData(57810, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bug off'.
  static const IconData bugOff = IconData(58641, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bug play'.
  static const IconData bugPlay = IconData(58642, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bug'.
  static const IconData bug = IconData(57867, fontFamily: 'LucideIcons');

  /// Lucide icon for 'building 2'.
  static const IconData building2 = IconData(57999, fontFamily: 'LucideIcons');

  /// Lucide icon for 'building'.
  static const IconData building = IconData(57803, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bus front'.
  static const IconData busFront = IconData(58623, fontFamily: 'LucideIcons');

  /// Lucide icon for 'bus'.
  static const IconData bus = IconData(57811, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cable car'.
  static const IconData cableCar = IconData(58624, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cable'.
  static const IconData cable = IconData(58599, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cake slice'.
  static const IconData cakeSlice = IconData(58557, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cake'.
  static const IconData cake = IconData(58183, fontFamily: 'LucideIcons');

  /// Lucide icon for 'calculator'.
  static const IconData calculator = IconData(57787, fontFamily: 'LucideIcons');

  /// Lucide icon for 'calendar 1'.
  static const IconData calendar1 = IconData(58932, fontFamily: 'LucideIcons');

  /// Lucide icon for 'calendar arrow down'.
  static const IconData calendarArrowDown = IconData(
    58882,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar arrow up'.
  static const IconData calendarArrowUp = IconData(
    58883,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar check 2'.
  static const IconData calendarCheck2 = IconData(
    58039,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar check'.
  static const IconData calendarCheck = IconData(
    58038,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar clock'.
  static const IconData calendarClock = IconData(
    58115,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar cog'.
  static const IconData calendarCog = IconData(
    58865,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar days'.
  static const IconData calendarDays = IconData(
    58040,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar fold'.
  static const IconData calendarFold = IconData(
    58808,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar heart'.
  static const IconData calendarHeart = IconData(
    58116,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar minus 2'.
  static const IconData calendarMinus2 = IconData(
    58809,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar minus'.
  static const IconData calendarMinus = IconData(
    58041,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar off'.
  static const IconData calendarOff = IconData(
    58042,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar plus 2'.
  static const IconData calendarPlus2 = IconData(
    58810,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar plus'.
  static const IconData calendarPlus = IconData(
    58043,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar range'.
  static const IconData calendarRange = IconData(
    58044,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar search'.
  static const IconData calendarSearch = IconData(
    58117,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar sync'.
  static const IconData calendarSync = IconData(
    58938,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'calendar X 2'.
  static const IconData calendarX2 = IconData(58046, fontFamily: 'LucideIcons');

  /// Lucide icon for 'calendar X'.
  static const IconData calendarX = IconData(58045, fontFamily: 'LucideIcons');

  /// Lucide icon for 'calendar'.
  static const IconData calendar = IconData(57447, fontFamily: 'LucideIcons');

  /// Lucide icon for 'camera off'.
  static const IconData cameraOff = IconData(57449, fontFamily: 'LucideIcons');

  /// Lucide icon for 'camera'.
  static const IconData camera = IconData(57448, fontFamily: 'LucideIcons');

  /// Lucide icon for 'candy cane'.
  static const IconData candyCane = IconData(58558, fontFamily: 'LucideIcons');

  /// Lucide icon for 'candy off'.
  static const IconData candyOff = IconData(58261, fontFamily: 'LucideIcons');

  /// Lucide icon for 'candy'.
  static const IconData candy = IconData(58260, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cannabis'.
  static const IconData cannabis = IconData(58840, fontFamily: 'LucideIcons');

  /// Lucide icon for 'captions off'.
  static const IconData captionsOff = IconData(
    58821,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'captions'.
  static const IconData captions = IconData(58279, fontFamily: 'LucideIcons');

  /// Lucide icon for 'car front'.
  static const IconData carFront = IconData(58625, fontFamily: 'LucideIcons');

  /// Lucide icon for 'car taxi front'.
  static const IconData carTaxiFront = IconData(
    58626,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'car'.
  static const IconData car = IconData(57812, fontFamily: 'LucideIcons');

  /// Lucide icon for 'caravan'.
  static const IconData caravan = IconData(58685, fontFamily: 'LucideIcons');

  /// Lucide icon for 'carrot'.
  static const IconData carrot = IconData(57945, fontFamily: 'LucideIcons');

  /// Lucide icon for 'case lower'.
  static const IconData caseLower = IconData(58331, fontFamily: 'LucideIcons');

  /// Lucide icon for 'case sensitive'.
  static const IconData caseSensitive = IconData(
    58332,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'case upper'.
  static const IconData caseUpper = IconData(58333, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cassette tape'.
  static const IconData cassetteTape = IconData(
    58574,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cast'.
  static const IconData cast = IconData(57450, fontFamily: 'LucideIcons');

  /// Lucide icon for 'castle'.
  static const IconData castle = IconData(58339, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cat'.
  static const IconData cat = IconData(58255, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cctv'.
  static const IconData cctv = IconData(58753, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chart area'.
  static const IconData chartArea = IconData(58583, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chart bar big'.
  static const IconData chartBarBig = IconData(
    58539,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart bar decreasing'.
  static const IconData chartBarDecreasing = IconData(
    58891,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart bar increasing'.
  static const IconData chartBarIncreasing = IconData(
    58892,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart bar stacked'.
  static const IconData chartBarStacked = IconData(
    58893,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart bar'.
  static const IconData chartBar = IconData(58017, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chart candlestick'.
  static const IconData chartCandlestick = IconData(
    58540,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart column big'.
  static const IconData chartColumnBig = IconData(
    58541,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart column decreasing'.
  static const IconData chartColumnDecreasing = IconData(
    57451,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart column increasing'.
  static const IconData chartColumnIncreasing = IconData(
    58019,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart column stacked'.
  static const IconData chartColumnStacked = IconData(
    58894,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart column'.
  static const IconData chartColumn = IconData(
    58018,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart gantt'.
  static const IconData chartGantt = IconData(58920, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chart line'.
  static const IconData chartLine = IconData(58020, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chart network'.
  static const IconData chartNetwork = IconData(
    58895,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart no axes column decreasing'.
  static const IconData chartNoAxesColumnDecreasing = IconData(
    57453,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart no axes column increasing'.
  static const IconData chartNoAxesColumnIncreasing = IconData(
    57454,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart no axes column'.
  static const IconData chartNoAxesColumn = IconData(
    57452,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart no axes combined'.
  static const IconData chartNoAxesCombined = IconData(
    58896,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart no axes gantt'.
  static const IconData chartNoAxesGantt = IconData(
    58568,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart pie'.
  static const IconData chartPie = IconData(57455, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chart scatter'.
  static const IconData chartScatter = IconData(
    58510,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chart spline'.
  static const IconData chartSpline = IconData(
    58897,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'check check'.
  static const IconData checkCheck = IconData(58257, fontFamily: 'LucideIcons');

  /// Lucide icon for 'check'.
  static const IconData check = IconData(57456, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chef hat'.
  static const IconData chefHat = IconData(58027, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cherry'.
  static const IconData cherry = IconData(58195, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chevron down'.
  static const IconData chevronDown = IconData(
    57457,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevron first'.
  static const IconData chevronFirst = IconData(
    57922,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevron last'.
  static const IconData chevronLast = IconData(
    57923,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevron left'.
  static const IconData chevronLeft = IconData(
    57458,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevron right'.
  static const IconData chevronRight = IconData(
    57459,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevron up'.
  static const IconData chevronUp = IconData(57460, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chevrons down up'.
  static const IconData chevronsDownUp = IconData(
    57895,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevrons down'.
  static const IconData chevronsDown = IconData(
    57461,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevrons left right ellipsis'.
  static const IconData chevronsLeftRightEllipsis = IconData(
    58915,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevrons left right'.
  static const IconData chevronsLeftRight = IconData(
    58002,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevrons left'.
  static const IconData chevronsLeft = IconData(
    57462,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevrons right left'.
  static const IconData chevronsRightLeft = IconData(
    58003,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevrons right'.
  static const IconData chevronsRight = IconData(
    57463,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevrons up down'.
  static const IconData chevronsUpDown = IconData(
    57872,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'chevrons up'.
  static const IconData chevronsUp = IconData(57464, fontFamily: 'LucideIcons');

  /// Lucide icon for 'chrome'.
  static const IconData chrome = IconData(57465, fontFamily: 'LucideIcons');

  /// Lucide icon for 'church'.
  static const IconData church = IconData(58340, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cigarette off'.
  static const IconData cigaretteOff = IconData(
    58054,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cigarette'.
  static const IconData cigarette = IconData(58053, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle alert'.
  static const IconData circleAlert = IconData(
    57467,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle arrow down'.
  static const IconData circleArrowDown = IconData(
    57468,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle arrow left'.
  static const IconData circleArrowLeft = IconData(
    57469,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle arrow out down left'.
  static const IconData circleArrowOutDownLeft = IconData(
    58363,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle arrow out down right'.
  static const IconData circleArrowOutDownRight = IconData(
    58364,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle arrow out up left'.
  static const IconData circleArrowOutUpLeft = IconData(
    58365,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle arrow out up right'.
  static const IconData circleArrowOutUpRight = IconData(
    58366,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle arrow right'.
  static const IconData circleArrowRight = IconData(
    57470,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle arrow up'.
  static const IconData circleArrowUp = IconData(
    57471,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle check big'.
  static const IconData circleCheckBig = IconData(
    57472,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle check'.
  static const IconData circleCheck = IconData(
    57893,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle chevron down'.
  static const IconData circleChevronDown = IconData(
    58593,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle chevron left'.
  static const IconData circleChevronLeft = IconData(
    58594,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle chevron right'.
  static const IconData circleChevronRight = IconData(
    58595,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle chevron up'.
  static const IconData circleChevronUp = IconData(
    58596,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle dashed'.
  static const IconData circleDashed = IconData(
    58548,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle divide'.
  static const IconData circleDivide = IconData(
    57473,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle dollar sign'.
  static const IconData circleDollarSign = IconData(
    58497,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle dot dashed'.
  static const IconData circleDotDashed = IconData(
    58549,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle dot'.
  static const IconData circleDot = IconData(58184, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle ellipsis'.
  static const IconData circleEllipsis = IconData(
    58185,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle equal'.
  static const IconData circleEqual = IconData(
    58372,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle fading arrow up'.
  static const IconData circleFadingArrowUp = IconData(
    58908,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle fading plus'.
  static const IconData circleFadingPlus = IconData(
    58816,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle gauge'.
  static const IconData circleGauge = IconData(
    58597,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle help'.
  static const IconData circleHelp = IconData(57474, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle minus'.
  static const IconData circleMinus = IconData(
    57475,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle off'.
  static const IconData circleOff = IconData(58373, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle parking off'.
  static const IconData circleParkingOff = IconData(
    58317,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle parking'.
  static const IconData circleParking = IconData(
    58316,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle pause'.
  static const IconData circlePause = IconData(
    57476,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle percent'.
  static const IconData circlePercent = IconData(
    58654,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle play'.
  static const IconData circlePlay = IconData(57477, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle plus'.
  static const IconData circlePlus = IconData(57478, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle power'.
  static const IconData circlePower = IconData(
    58708,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle slash 2'.
  static const IconData circleSlash2 = IconData(
    57874,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle slash'.
  static const IconData circleSlash = IconData(
    58374,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle small'.
  static const IconData circleSmall = IconData(
    58948,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle stop'.
  static const IconData circleStop = IconData(57479, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle user round'.
  static const IconData circleUserRound = IconData(
    58470,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'circle user'.
  static const IconData circleUser = IconData(58469, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle X'.
  static const IconData circleX = IconData(57480, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circle'.
  static const IconData circle = IconData(57466, fontFamily: 'LucideIcons');

  /// Lucide icon for 'circuit board'.
  static const IconData circuitBoard = IconData(
    58375,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'citrus'.
  static const IconData citrus = IconData(58232, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clapperboard'.
  static const IconData clapperboard = IconData(
    58010,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard check'.
  static const IconData clipboardCheck = IconData(
    57880,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard copy'.
  static const IconData clipboardCopy = IconData(
    57892,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard list'.
  static const IconData clipboardList = IconData(
    57482,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard minus'.
  static const IconData clipboardMinus = IconData(
    58818,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard paste'.
  static const IconData clipboardPaste = IconData(
    58347,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard pen line'.
  static const IconData clipboardPenLine = IconData(
    58119,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard pen'.
  static const IconData clipboardPen = IconData(
    58118,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard plus'.
  static const IconData clipboardPlus = IconData(
    58819,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard type'.
  static const IconData clipboardType = IconData(
    58120,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clipboard X'.
  static const IconData clipboardX = IconData(57889, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clipboard'.
  static const IconData clipboard = IconData(57481, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 1'.
  static const IconData clock1 = IconData(57930, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 10'.
  static const IconData clock10 = IconData(57931, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 11'.
  static const IconData clock11 = IconData(57932, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 12'.
  static const IconData clock12 = IconData(57933, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 2'.
  static const IconData clock2 = IconData(57934, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 3'.
  static const IconData clock3 = IconData(57935, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 4'.
  static const IconData clock4 = IconData(57936, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 5'.
  static const IconData clock5 = IconData(57937, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 6'.
  static const IconData clock6 = IconData(57938, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 7'.
  static const IconData clock7 = IconData(57939, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 8'.
  static const IconData clock8 = IconData(57940, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock 9'.
  static const IconData clock9 = IconData(57941, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock alert'.
  static const IconData clockAlert = IconData(58926, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clock arrow down'.
  static const IconData clockArrowDown = IconData(
    58884,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clock arrow up'.
  static const IconData clockArrowUp = IconData(
    58885,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'clock'.
  static const IconData clock = IconData(57483, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud alert'.
  static const IconData cloudAlert = IconData(58935, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud cog'.
  static const IconData cloudCog = IconData(58121, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud download'.
  static const IconData cloudDownload = IconData(
    57485,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cloud drizzle'.
  static const IconData cloudDrizzle = IconData(
    57486,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cloud fog'.
  static const IconData cloudFog = IconData(57875, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud hail'.
  static const IconData cloudHail = IconData(57487, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud lightning'.
  static const IconData cloudLightning = IconData(
    57488,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cloud moon rain'.
  static const IconData cloudMoonRain = IconData(
    58105,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cloud moon'.
  static const IconData cloudMoon = IconData(57876, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud off'.
  static const IconData cloudOff = IconData(57489, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud rain wind'.
  static const IconData cloudRainWind = IconData(
    57491,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cloud rain'.
  static const IconData cloudRain = IconData(57490, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud snow'.
  static const IconData cloudSnow = IconData(57492, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud sun rain'.
  static const IconData cloudSunRain = IconData(
    58106,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cloud sun'.
  static const IconData cloudSun = IconData(57877, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloud upload'.
  static const IconData cloudUpload = IconData(
    57493,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cloud'.
  static const IconData cloud = IconData(57484, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cloudy'.
  static const IconData cloudy = IconData(57878, fontFamily: 'LucideIcons');

  /// Lucide icon for 'clover'.
  static const IconData clover = IconData(57494, fontFamily: 'LucideIcons');

  /// Lucide icon for 'club'.
  static const IconData club = IconData(58522, fontFamily: 'LucideIcons');

  /// Lucide icon for 'code xml'.
  static const IconData codeXml = IconData(57861, fontFamily: 'LucideIcons');

  /// Lucide icon for 'code'.
  static const IconData code = IconData(57495, fontFamily: 'LucideIcons');

  /// Lucide icon for 'codepen'.
  static const IconData codepen = IconData(57496, fontFamily: 'LucideIcons');

  /// Lucide icon for 'codesandbox'.
  static const IconData codesandbox = IconData(
    57497,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'coffee'.
  static const IconData coffee = IconData(57498, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cog'.
  static const IconData cog = IconData(58122, fontFamily: 'LucideIcons');

  /// Lucide icon for 'coins'.
  static const IconData coins = IconData(57499, fontFamily: 'LucideIcons');

  /// Lucide icon for 'columns 2'.
  static const IconData columns2 = IconData(57500, fontFamily: 'LucideIcons');

  /// Lucide icon for 'columns 3'.
  static const IconData columns3 = IconData(57501, fontFamily: 'LucideIcons');

  /// Lucide icon for 'columns 4'.
  static const IconData columns4 = IconData(58765, fontFamily: 'LucideIcons');

  /// Lucide icon for 'combine'.
  static const IconData combine = IconData(58448, fontFamily: 'LucideIcons');

  /// Lucide icon for 'command'.
  static const IconData command = IconData(57502, fontFamily: 'LucideIcons');

  /// Lucide icon for 'compass'.
  static const IconData compass = IconData(57503, fontFamily: 'LucideIcons');

  /// Lucide icon for 'component'.
  static const IconData component = IconData(58028, fontFamily: 'LucideIcons');

  /// Lucide icon for 'computer'.
  static const IconData computer = IconData(58600, fontFamily: 'LucideIcons');

  /// Lucide icon for 'concierge bell'.
  static const IconData conciergeBell = IconData(
    58235,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cone'.
  static const IconData cone = IconData(58663, fontFamily: 'LucideIcons');

  /// Lucide icon for 'construction'.
  static const IconData construction = IconData(
    58295,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'contact round'.
  static const IconData contactRound = IconData(
    58471,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'contact'.
  static const IconData contact = IconData(57504, fontFamily: 'LucideIcons');

  /// Lucide icon for 'container'.
  static const IconData container = IconData(58585, fontFamily: 'LucideIcons');

  /// Lucide icon for 'contrast'.
  static const IconData contrast = IconData(57505, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cookie'.
  static const IconData cookie = IconData(57962, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cooking pot'.
  static const IconData cookingPot = IconData(58760, fontFamily: 'LucideIcons');

  /// Lucide icon for 'copy check'.
  static const IconData copyCheck = IconData(58367, fontFamily: 'LucideIcons');

  /// Lucide icon for 'copy minus'.
  static const IconData copyMinus = IconData(58368, fontFamily: 'LucideIcons');

  /// Lucide icon for 'copy plus'.
  static const IconData copyPlus = IconData(58369, fontFamily: 'LucideIcons');

  /// Lucide icon for 'copy slash'.
  static const IconData copySlash = IconData(58370, fontFamily: 'LucideIcons');

  /// Lucide icon for 'copy X'.
  static const IconData copyX = IconData(58371, fontFamily: 'LucideIcons');

  /// Lucide icon for 'copy'.
  static const IconData copy = IconData(57506, fontFamily: 'LucideIcons');

  /// Lucide icon for 'copyleft'.
  static const IconData copyleft = IconData(57507, fontFamily: 'LucideIcons');

  /// Lucide icon for 'copyright'.
  static const IconData copyright = IconData(57508, fontFamily: 'LucideIcons');

  /// Lucide icon for 'corner down left'.
  static const IconData cornerDownLeft = IconData(
    57509,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'corner down right'.
  static const IconData cornerDownRight = IconData(
    57510,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'corner left down'.
  static const IconData cornerLeftDown = IconData(
    57511,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'corner left up'.
  static const IconData cornerLeftUp = IconData(
    57512,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'corner right down'.
  static const IconData cornerRightDown = IconData(
    57513,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'corner right up'.
  static const IconData cornerRightUp = IconData(
    57514,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'corner up left'.
  static const IconData cornerUpLeft = IconData(
    57515,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'corner up right'.
  static const IconData cornerUpRight = IconData(
    57516,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'cpu'.
  static const IconData cpu = IconData(57517, fontFamily: 'LucideIcons');

  /// Lucide icon for 'creative commons'.
  static const IconData creativeCommons = IconData(
    58293,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'credit card'.
  static const IconData creditCard = IconData(57518, fontFamily: 'LucideIcons');

  /// Lucide icon for 'croissant'.
  static const IconData croissant = IconData(58029, fontFamily: 'LucideIcons');

  /// Lucide icon for 'crop'.
  static const IconData crop = IconData(57519, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cross'.
  static const IconData cross = IconData(57828, fontFamily: 'LucideIcons');

  /// Lucide icon for 'crosshair'.
  static const IconData crosshair = IconData(57520, fontFamily: 'LucideIcons');

  /// Lucide icon for 'crown'.
  static const IconData crown = IconData(57813, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cuboid'.
  static const IconData cuboid = IconData(58664, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cup soda'.
  static const IconData cupSoda = IconData(58064, fontFamily: 'LucideIcons');

  /// Lucide icon for 'currency'.
  static const IconData currency = IconData(57903, fontFamily: 'LucideIcons');

  /// Lucide icon for 'cylinder'.
  static const IconData cylinder = IconData(58665, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dam'.
  static const IconData dam = IconData(58890, fontFamily: 'LucideIcons');

  /// Lucide icon for 'database backup'.
  static const IconData databaseBackup = IconData(
    58286,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'database zap'.
  static const IconData databaseZap = IconData(
    58639,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'database'.
  static const IconData database = IconData(57521, fontFamily: 'LucideIcons');

  /// Lucide icon for 'delete'.
  static const IconData delete = IconData(57522, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dessert'.
  static const IconData dessert = IconData(58559, fontFamily: 'LucideIcons');

  /// Lucide icon for 'diameter'.
  static const IconData diameter = IconData(58666, fontFamily: 'LucideIcons');

  /// Lucide icon for 'diamond minus'.
  static const IconData diamondMinus = IconData(
    58853,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'diamond percent'.
  static const IconData diamondPercent = IconData(
    58655,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'diamond plus'.
  static const IconData diamondPlus = IconData(
    58854,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'diamond'.
  static const IconData diamond = IconData(58065, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dice 1'.
  static const IconData dice1 = IconData(57990, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dice 2'.
  static const IconData dice2 = IconData(57991, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dice 3'.
  static const IconData dice3 = IconData(57992, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dice 4'.
  static const IconData dice4 = IconData(57993, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dice 5'.
  static const IconData dice5 = IconData(57994, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dice 6'.
  static const IconData dice6 = IconData(57995, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dices'.
  static const IconData dices = IconData(58052, fontFamily: 'LucideIcons');

  /// Lucide icon for 'diff'.
  static const IconData diff = IconData(58123, fontFamily: 'LucideIcons');

  /// Lucide icon for 'disc 2'.
  static const IconData disc2 = IconData(58362, fontFamily: 'LucideIcons');

  /// Lucide icon for 'disc 3'.
  static const IconData disc3 = IconData(58520, fontFamily: 'LucideIcons');

  /// Lucide icon for 'disc album'.
  static const IconData discAlbum = IconData(58720, fontFamily: 'LucideIcons');

  /// Lucide icon for 'disc'.
  static const IconData disc = IconData(57523, fontFamily: 'LucideIcons');

  /// Lucide icon for 'divide'.
  static const IconData divide = IconData(57524, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dna off'.
  static const IconData dnaOff = IconData(58263, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dna'.
  static const IconData dna = IconData(58262, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dock'.
  static const IconData dock = IconData(58839, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dog'.
  static const IconData dog = IconData(58256, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dollar sign'.
  static const IconData dollarSign = IconData(57525, fontFamily: 'LucideIcons');

  /// Lucide icon for 'donut'.
  static const IconData donut = IconData(58560, fontFamily: 'LucideIcons');

  /// Lucide icon for 'door closed'.
  static const IconData doorClosed = IconData(58328, fontFamily: 'LucideIcons');

  /// Lucide icon for 'door open'.
  static const IconData doorOpen = IconData(58329, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dot'.
  static const IconData dot = IconData(58451, fontFamily: 'LucideIcons');

  /// Lucide icon for 'download'.
  static const IconData download = IconData(57526, fontFamily: 'LucideIcons');

  /// Lucide icon for 'drafting compass'.
  static const IconData draftingCompass = IconData(
    58667,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'drama'.
  static const IconData drama = IconData(58661, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dribbble'.
  static const IconData dribbble = IconData(57527, fontFamily: 'LucideIcons');

  /// Lucide icon for 'drill'.
  static const IconData drill = IconData(58769, fontFamily: 'LucideIcons');

  /// Lucide icon for 'droplet off'.
  static const IconData dropletOff = IconData(58940, fontFamily: 'LucideIcons');

  /// Lucide icon for 'droplet'.
  static const IconData droplet = IconData(57528, fontFamily: 'LucideIcons');

  /// Lucide icon for 'droplets'.
  static const IconData droplets = IconData(57529, fontFamily: 'LucideIcons');

  /// Lucide icon for 'drum'.
  static const IconData drum = IconData(58721, fontFamily: 'LucideIcons');

  /// Lucide icon for 'drumstick'.
  static const IconData drumstick = IconData(57946, fontFamily: 'LucideIcons');

  /// Lucide icon for 'dumbbell'.
  static const IconData dumbbell = IconData(58276, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ear off'.
  static const IconData earOff = IconData(58246, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ear'.
  static const IconData ear = IconData(58245, fontFamily: 'LucideIcons');

  /// Lucide icon for 'earth lock'.
  static const IconData earthLock = IconData(58832, fontFamily: 'LucideIcons');

  /// Lucide icon for 'earth'.
  static const IconData earth = IconData(57842, fontFamily: 'LucideIcons');

  /// Lucide icon for 'eclipse'.
  static const IconData eclipse = IconData(58785, fontFamily: 'LucideIcons');

  /// Lucide icon for 'egg fried'.
  static const IconData eggFried = IconData(58196, fontFamily: 'LucideIcons');

  /// Lucide icon for 'egg off'.
  static const IconData eggOff = IconData(58264, fontFamily: 'LucideIcons');

  /// Lucide icon for 'egg'.
  static const IconData egg = IconData(57948, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ellipsis vertical'.
  static const IconData ellipsisVertical = IconData(
    57531,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ellipsis'.
  static const IconData ellipsis = IconData(57530, fontFamily: 'LucideIcons');

  /// Lucide icon for 'equal approximately'.
  static const IconData equalApproximately = IconData(
    58936,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'equal not'.
  static const IconData equalNot = IconData(57789, fontFamily: 'LucideIcons');

  /// Lucide icon for 'equal'.
  static const IconData equal = IconData(57788, fontFamily: 'LucideIcons');

  /// Lucide icon for 'eraser'.
  static const IconData eraser = IconData(57998, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ethernet port'.
  static const IconData ethernetPort = IconData(
    58916,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'euro'.
  static const IconData euro = IconData(57532, fontFamily: 'LucideIcons');

  /// Lucide icon for 'expand'.
  static const IconData expand = IconData(57881, fontFamily: 'LucideIcons');

  /// Lucide icon for 'external link'.
  static const IconData externalLink = IconData(
    57533,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'eye closed'.
  static const IconData eyeClosed = IconData(58930, fontFamily: 'LucideIcons');

  /// Lucide icon for 'eye off'.
  static const IconData eyeOff = IconData(57535, fontFamily: 'LucideIcons');

  /// Lucide icon for 'eye'.
  static const IconData eye = IconData(57534, fontFamily: 'LucideIcons');

  /// Lucide icon for 'facebook'.
  static const IconData facebook = IconData(57536, fontFamily: 'LucideIcons');

  /// Lucide icon for 'factory'.
  static const IconData factory = IconData(58014, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fan'.
  static const IconData fan = IconData(58236, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fast forward'.
  static const IconData fastForward = IconData(
    57537,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'feather'.
  static const IconData feather = IconData(57538, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fence'.
  static const IconData fence = IconData(58758, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ferris wheel'.
  static const IconData ferrisWheel = IconData(
    58499,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'figma'.
  static const IconData figma = IconData(57539, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file archive'.
  static const IconData fileArchive = IconData(
    58124,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file audio 2'.
  static const IconData fileAudio2 = IconData(58126, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file audio'.
  static const IconData fileAudio = IconData(58125, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file axis 3d'.
  static const IconData fileAxis3d = IconData(58127, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file badge 2'.
  static const IconData fileBadge2 = IconData(58129, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file badge'.
  static const IconData fileBadge = IconData(58128, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file box'.
  static const IconData fileBox = IconData(58130, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file chart column increasing'.
  static const IconData fileChartColumnIncreasing = IconData(
    58132,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file chart column'.
  static const IconData fileChartColumn = IconData(
    58131,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file chart line'.
  static const IconData fileChartLine = IconData(
    58133,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file chart pie'.
  static const IconData fileChartPie = IconData(
    58134,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file check 2'.
  static const IconData fileCheck2 = IconData(57542, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file check'.
  static const IconData fileCheck = IconData(57541, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file clock'.
  static const IconData fileClock = IconData(58135, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file code 2'.
  static const IconData fileCode2 = IconData(58466, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file code'.
  static const IconData fileCode = IconData(57543, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file cog'.
  static const IconData fileCog = IconData(58136, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file diff'.
  static const IconData fileDiff = IconData(58137, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file digit'.
  static const IconData fileDigit = IconData(57544, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file down'.
  static const IconData fileDown = IconData(58138, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file heart'.
  static const IconData fileHeart = IconData(58139, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file image'.
  static const IconData fileImage = IconData(58140, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file input'.
  static const IconData fileInput = IconData(57545, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file json 2'.
  static const IconData fileJson2 = IconData(58223, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file json'.
  static const IconData fileJson = IconData(58222, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file key 2'.
  static const IconData fileKey2 = IconData(58142, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file key'.
  static const IconData fileKey = IconData(58141, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file lock 2'.
  static const IconData fileLock2 = IconData(58144, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file lock'.
  static const IconData fileLock = IconData(58143, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file minus 2'.
  static const IconData fileMinus2 = IconData(57547, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file minus'.
  static const IconData fileMinus = IconData(57546, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file music'.
  static const IconData fileMusic = IconData(58722, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file output'.
  static const IconData fileOutput = IconData(57548, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file pen line'.
  static const IconData filePenLine = IconData(
    58146,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file pen'.
  static const IconData filePen = IconData(58145, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file plus 2'.
  static const IconData filePlus2 = IconData(57550, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file plus'.
  static const IconData filePlus = IconData(57549, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file question'.
  static const IconData fileQuestion = IconData(
    58147,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file scan'.
  static const IconData fileScan = IconData(58148, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file search 2'.
  static const IconData fileSearch2 = IconData(
    58149,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file search'.
  static const IconData fileSearch = IconData(57551, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file sliders'.
  static const IconData fileSliders = IconData(
    58788,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file spreadsheet'.
  static const IconData fileSpreadsheet = IconData(
    58150,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file stack'.
  static const IconData fileStack = IconData(58533, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file symlink'.
  static const IconData fileSymlink = IconData(
    58151,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file terminal'.
  static const IconData fileTerminal = IconData(
    58152,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file text'.
  static const IconData fileText = IconData(57552, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file type 2'.
  static const IconData fileType2 = IconData(58224, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file type'.
  static const IconData fileType = IconData(58153, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file up'.
  static const IconData fileUp = IconData(58154, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file user'.
  static const IconData fileUser = IconData(58929, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file video 2'.
  static const IconData fileVideo2 = IconData(58156, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file video'.
  static const IconData fileVideo = IconData(58155, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file volume 2'.
  static const IconData fileVolume2 = IconData(
    58158,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file volume'.
  static const IconData fileVolume = IconData(58157, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file warning'.
  static const IconData fileWarning = IconData(
    58159,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'file X 2'.
  static const IconData fileX2 = IconData(57554, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file X'.
  static const IconData fileX = IconData(57553, fontFamily: 'LucideIcons');

  /// Lucide icon for 'file'.
  static const IconData file = IconData(57540, fontFamily: 'LucideIcons');

  /// Lucide icon for 'files'.
  static const IconData files = IconData(57555, fontFamily: 'LucideIcons');

  /// Lucide icon for 'film'.
  static const IconData film = IconData(57556, fontFamily: 'LucideIcons');

  /// Lucide icon for 'filter X'.
  static const IconData filterX = IconData(58296, fontFamily: 'LucideIcons');

  /// Lucide icon for 'filter'.
  static const IconData filter = IconData(57557, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fingerprint'.
  static const IconData fingerprint = IconData(
    58058,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'fire extinguisher'.
  static const IconData fireExtinguisher = IconData(
    58754,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'fish off'.
  static const IconData fishOff = IconData(58291, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fish symbol'.
  static const IconData fishSymbol = IconData(58616, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fish'.
  static const IconData fish = IconData(58281, fontFamily: 'LucideIcons');

  /// Lucide icon for 'flag off'.
  static const IconData flagOff = IconData(58001, fontFamily: 'LucideIcons');

  /// Lucide icon for 'flag triangle left'.
  static const IconData flagTriangleLeft = IconData(
    57910,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flag triangle right'.
  static const IconData flagTriangleRight = IconData(
    57911,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flag'.
  static const IconData flag = IconData(57558, fontFamily: 'LucideIcons');

  /// Lucide icon for 'flame kindling'.
  static const IconData flameKindling = IconData(
    58686,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flame'.
  static const IconData flame = IconData(57559, fontFamily: 'LucideIcons');

  /// Lucide icon for 'flashlight off'.
  static const IconData flashlightOff = IconData(
    57561,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flashlight'.
  static const IconData flashlight = IconData(57560, fontFamily: 'LucideIcons');

  /// Lucide icon for 'flask conical off'.
  static const IconData flaskConicalOff = IconData(
    58265,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flask conical'.
  static const IconData flaskConical = IconData(
    57562,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flask round'.
  static const IconData flaskRound = IconData(57563, fontFamily: 'LucideIcons');

  /// Lucide icon for 'flip horizontal 2'.
  static const IconData flipHorizontal2 = IconData(
    58209,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flip horizontal'.
  static const IconData flipHorizontal = IconData(
    58208,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flip vertical 2'.
  static const IconData flipVertical2 = IconData(
    58211,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flip vertical'.
  static const IconData flipVertical = IconData(
    58210,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'flower 2'.
  static const IconData flower2 = IconData(58067, fontFamily: 'LucideIcons');

  /// Lucide icon for 'flower'.
  static const IconData flower = IconData(58066, fontFamily: 'LucideIcons');

  /// Lucide icon for 'focus'.
  static const IconData focus = IconData(58013, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fold horizontal'.
  static const IconData foldHorizontal = IconData(
    58431,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'fold vertical'.
  static const IconData foldVertical = IconData(
    58432,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder archive'.
  static const IconData folderArchive = IconData(
    58160,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder check'.
  static const IconData folderCheck = IconData(
    58161,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder clock'.
  static const IconData folderClock = IconData(
    58162,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder closed'.
  static const IconData folderClosed = IconData(
    58163,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder code'.
  static const IconData folderCode = IconData(58879, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder cog'.
  static const IconData folderCog = IconData(58164, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder dot'.
  static const IconData folderDot = IconData(58569, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder down'.
  static const IconData folderDown = IconData(58165, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder git 2'.
  static const IconData folderGit2 = IconData(58382, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder git'.
  static const IconData folderGit = IconData(58381, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder heart'.
  static const IconData folderHeart = IconData(
    58166,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder input'.
  static const IconData folderInput = IconData(
    58167,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder kanban'.
  static const IconData folderKanban = IconData(
    58570,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder key'.
  static const IconData folderKey = IconData(58168, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder lock'.
  static const IconData folderLock = IconData(58169, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder minus'.
  static const IconData folderMinus = IconData(
    57565,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder open dot'.
  static const IconData folderOpenDot = IconData(
    58571,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder open'.
  static const IconData folderOpen = IconData(57926, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder output'.
  static const IconData folderOutput = IconData(
    58170,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder pen'.
  static const IconData folderPen = IconData(58171, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder plus'.
  static const IconData folderPlus = IconData(57566, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder root'.
  static const IconData folderRoot = IconData(58572, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder search 2'.
  static const IconData folderSearch2 = IconData(
    58173,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder search'.
  static const IconData folderSearch = IconData(
    58172,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder symlink'.
  static const IconData folderSymlink = IconData(
    58174,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'folder sync'.
  static const IconData folderSync = IconData(58573, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder tree'.
  static const IconData folderTree = IconData(58175, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder up'.
  static const IconData folderUp = IconData(58176, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder X'.
  static const IconData folderX = IconData(58177, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folder'.
  static const IconData folder = IconData(57564, fontFamily: 'LucideIcons');

  /// Lucide icon for 'folders'.
  static const IconData folders = IconData(58178, fontFamily: 'LucideIcons');

  /// Lucide icon for 'footprints'.
  static const IconData footprints = IconData(58300, fontFamily: 'LucideIcons');

  /// Lucide icon for 'forklift'.
  static const IconData forklift = IconData(58308, fontFamily: 'LucideIcons');

  /// Lucide icon for 'forward'.
  static const IconData forward = IconData(57896, fontFamily: 'LucideIcons');

  /// Lucide icon for 'frame'.
  static const IconData frame = IconData(58000, fontFamily: 'LucideIcons');

  /// Lucide icon for 'framer'.
  static const IconData framer = IconData(57567, fontFamily: 'LucideIcons');

  /// Lucide icon for 'frown'.
  static const IconData frown = IconData(57568, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fuel'.
  static const IconData fuel = IconData(58030, fontFamily: 'LucideIcons');

  /// Lucide icon for 'fullscreen'.
  static const IconData fullscreen = IconData(58680, fontFamily: 'LucideIcons');

  /// Lucide icon for 'gallery horizontal end'.
  static const IconData galleryHorizontalEnd = IconData(
    58579,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'gallery horizontal'.
  static const IconData galleryHorizontal = IconData(
    58578,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'gallery thumbnails'.
  static const IconData galleryThumbnails = IconData(
    58580,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'gallery vertical end'.
  static const IconData galleryVerticalEnd = IconData(
    58582,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'gallery vertical'.
  static const IconData galleryVertical = IconData(
    58581,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'gamepad 2'.
  static const IconData gamepad2 = IconData(57570, fontFamily: 'LucideIcons');

  /// Lucide icon for 'gamepad'.
  static const IconData gamepad = IconData(57569, fontFamily: 'LucideIcons');

  /// Lucide icon for 'gauge'.
  static const IconData gauge = IconData(57790, fontFamily: 'LucideIcons');

  /// Lucide icon for 'gavel'.
  static const IconData gavel = IconData(57571, fontFamily: 'LucideIcons');

  /// Lucide icon for 'gem'.
  static const IconData gem = IconData(57921, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ghost'.
  static const IconData ghost = IconData(57869, fontFamily: 'LucideIcons');

  /// Lucide icon for 'gift'.
  static const IconData gift = IconData(57572, fontFamily: 'LucideIcons');

  /// Lucide icon for 'git branch plus'.
  static const IconData gitBranchPlus = IconData(
    57843,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git branch'.
  static const IconData gitBranch = IconData(57573, fontFamily: 'LucideIcons');

  /// Lucide icon for 'git commit horizontal'.
  static const IconData gitCommitHorizontal = IconData(
    57574,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git commit vertical'.
  static const IconData gitCommitVertical = IconData(
    58710,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git compare arrows'.
  static const IconData gitCompareArrows = IconData(
    58711,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git compare'.
  static const IconData gitCompare = IconData(58204, fontFamily: 'LucideIcons');

  /// Lucide icon for 'git fork'.
  static const IconData gitFork = IconData(57996, fontFamily: 'LucideIcons');

  /// Lucide icon for 'git graph'.
  static const IconData gitGraph = IconData(58712, fontFamily: 'LucideIcons');

  /// Lucide icon for 'git merge'.
  static const IconData gitMerge = IconData(57575, fontFamily: 'LucideIcons');

  /// Lucide icon for 'git pull request arrow'.
  static const IconData gitPullRequestArrow = IconData(
    58713,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git pull request closed'.
  static const IconData gitPullRequestClosed = IconData(
    58205,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git pull request create arrow'.
  static const IconData gitPullRequestCreateArrow = IconData(
    58715,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git pull request create'.
  static const IconData gitPullRequestCreate = IconData(
    58714,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git pull request draft'.
  static const IconData gitPullRequestDraft = IconData(
    58206,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'git pull request'.
  static const IconData gitPullRequest = IconData(
    57576,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'github'.
  static const IconData github = IconData(57577, fontFamily: 'LucideIcons');

  /// Lucide icon for 'gitlab'.
  static const IconData gitlab = IconData(57578, fontFamily: 'LucideIcons');

  /// Lucide icon for 'glass water'.
  static const IconData glassWater = IconData(58068, fontFamily: 'LucideIcons');

  /// Lucide icon for 'glasses'.
  static const IconData glasses = IconData(57868, fontFamily: 'LucideIcons');

  /// Lucide icon for 'globe lock'.
  static const IconData globeLock = IconData(58833, fontFamily: 'LucideIcons');

  /// Lucide icon for 'globe'.
  static const IconData globe = IconData(57579, fontFamily: 'LucideIcons');

  /// Lucide icon for 'goal'.
  static const IconData goal = IconData(58537, fontFamily: 'LucideIcons');

  /// Lucide icon for 'grab'.
  static const IconData grab = IconData(57829, fontFamily: 'LucideIcons');

  /// Lucide icon for 'graduation cap'.
  static const IconData graduationCap = IconData(
    57907,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'grape'.
  static const IconData grape = IconData(58197, fontFamily: 'LucideIcons');

  /// Lucide icon for 'grid 2x 2 check'.
  static const IconData grid2x2Check = IconData(
    58856,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'grid 2x 2 plus'.
  static const IconData grid2x2Plus = IconData(
    58924,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'grid 2x 2 X'.
  static const IconData grid2x2X = IconData(58857, fontFamily: 'LucideIcons');

  /// Lucide icon for 'grid 2x 2'.
  static const IconData grid2x2 = IconData(58627, fontFamily: 'LucideIcons');

  /// Lucide icon for 'grid 3x 3'.
  static const IconData grid3x3 = IconData(57580, fontFamily: 'LucideIcons');

  /// Lucide icon for 'grip horizontal'.
  static const IconData gripHorizontal = IconData(
    57581,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'grip vertical'.
  static const IconData gripVertical = IconData(
    57582,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'grip'.
  static const IconData grip = IconData(58292, fontFamily: 'LucideIcons');

  /// Lucide icon for 'group'.
  static const IconData group = IconData(58472, fontFamily: 'LucideIcons');

  /// Lucide icon for 'guitar'.
  static const IconData guitar = IconData(58723, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ham'.
  static const IconData ham = IconData(58843, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hammer'.
  static const IconData hammer = IconData(57583, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hand coins'.
  static const IconData handCoins = IconData(58812, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hand heart'.
  static const IconData handHeart = IconData(58813, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hand helping'.
  static const IconData handHelping = IconData(
    58299,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'hand metal'.
  static const IconData handMetal = IconData(57899, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hand platter'.
  static const IconData handPlatter = IconData(
    58814,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'hand'.
  static const IconData hand = IconData(57814, fontFamily: 'LucideIcons');

  /// Lucide icon for 'handshake'.
  static const IconData handshake = IconData(58820, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hard drive download'.
  static const IconData hardDriveDownload = IconData(
    58601,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'hard drive upload'.
  static const IconData hardDriveUpload = IconData(
    58602,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'hard drive'.
  static const IconData hardDrive = IconData(57584, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hard hat'.
  static const IconData hardHat = IconData(57585, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hash'.
  static const IconData hash = IconData(57586, fontFamily: 'LucideIcons');

  /// Lucide icon for 'haze'.
  static const IconData haze = IconData(57587, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hdmi port'.
  static const IconData hdmiPort = IconData(58603, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heading 1'.
  static const IconData heading1 = IconData(58248, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heading 2'.
  static const IconData heading2 = IconData(58249, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heading 3'.
  static const IconData heading3 = IconData(58250, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heading 4'.
  static const IconData heading4 = IconData(58251, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heading 5'.
  static const IconData heading5 = IconData(58252, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heading 6'.
  static const IconData heading6 = IconData(58253, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heading'.
  static const IconData heading = IconData(58247, fontFamily: 'LucideIcons');

  /// Lucide icon for 'headphone off'.
  static const IconData headphoneOff = IconData(
    58925,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'headphones'.
  static const IconData headphones = IconData(57588, fontFamily: 'LucideIcons');

  /// Lucide icon for 'headset'.
  static const IconData headset = IconData(58817, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heart crack'.
  static const IconData heartCrack = IconData(58069, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heart handshake'.
  static const IconData heartHandshake = IconData(
    58070,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'heart off'.
  static const IconData heartOff = IconData(58004, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heart pulse'.
  static const IconData heartPulse = IconData(58225, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heart'.
  static const IconData heart = IconData(57589, fontFamily: 'LucideIcons');

  /// Lucide icon for 'heater'.
  static const IconData heater = IconData(58770, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hexagon'.
  static const IconData hexagon = IconData(57590, fontFamily: 'LucideIcons');

  /// Lucide icon for 'highlighter'.
  static const IconData highlighter = IconData(
    57591,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'history'.
  static const IconData history = IconData(57844, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hop off'.
  static const IconData hopOff = IconData(58267, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hop'.
  static const IconData hop = IconData(58266, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hospital'.
  static const IconData hospital = IconData(58844, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hotel'.
  static const IconData hotel = IconData(58341, fontFamily: 'LucideIcons');

  /// Lucide icon for 'hourglass'.
  static const IconData hourglass = IconData(58005, fontFamily: 'LucideIcons');

  /// Lucide icon for 'house plug'.
  static const IconData housePlug = IconData(58868, fontFamily: 'LucideIcons');

  /// Lucide icon for 'house plus'.
  static const IconData housePlus = IconData(58869, fontFamily: 'LucideIcons');

  /// Lucide icon for 'house wifi'.
  static const IconData houseWifi = IconData(58944, fontFamily: 'LucideIcons');

  /// Lucide icon for 'house'.
  static const IconData house = IconData(57592, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ice cream bowl'.
  static const IconData iceCreamBowl = IconData(
    58282,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ice cream cone'.
  static const IconData iceCreamCone = IconData(
    58198,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'id card'.
  static const IconData idCard = IconData(58907, fontFamily: 'LucideIcons');

  /// Lucide icon for 'image down'.
  static const IconData imageDown = IconData(58688, fontFamily: 'LucideIcons');

  /// Lucide icon for 'image minus'.
  static const IconData imageMinus = IconData(57845, fontFamily: 'LucideIcons');

  /// Lucide icon for 'image off'.
  static const IconData imageOff = IconData(57791, fontFamily: 'LucideIcons');

  /// Lucide icon for 'image play'.
  static const IconData imagePlay = IconData(58851, fontFamily: 'LucideIcons');

  /// Lucide icon for 'image plus'.
  static const IconData imagePlus = IconData(57846, fontFamily: 'LucideIcons');

  /// Lucide icon for 'image up'.
  static const IconData imageUp = IconData(58831, fontFamily: 'LucideIcons');

  /// Lucide icon for 'image upscale'.
  static const IconData imageUpscale = IconData(
    58939,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'image'.
  static const IconData image = IconData(57593, fontFamily: 'LucideIcons');

  /// Lucide icon for 'images'.
  static const IconData images = IconData(58824, fontFamily: 'LucideIcons');

  /// Lucide icon for 'import'.
  static const IconData import = IconData(57902, fontFamily: 'LucideIcons');

  /// Lucide icon for 'inbox'.
  static const IconData inbox = IconData(57594, fontFamily: 'LucideIcons');

  /// Lucide icon for 'indent decrease'.
  static const IconData indentDecrease = IconData(
    57595,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'indent increase'.
  static const IconData indentIncrease = IconData(
    57596,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'indian rupee'.
  static const IconData indianRupee = IconData(
    57597,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'infinity'.
  static const IconData infinity = IconData(57830, fontFamily: 'LucideIcons');

  /// Lucide icon for 'info'.
  static const IconData info = IconData(57598, fontFamily: 'LucideIcons');

  /// Lucide icon for 'inspection panel'.
  static const IconData inspectionPanel = IconData(
    58759,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'instagram'.
  static const IconData instagram = IconData(57599, fontFamily: 'LucideIcons');

  /// Lucide icon for 'italic'.
  static const IconData italic = IconData(57600, fontFamily: 'LucideIcons');

  /// Lucide icon for 'iteration ccw'.
  static const IconData iterationCcw = IconData(
    58407,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'iteration cw'.
  static const IconData iterationCw = IconData(
    58408,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'japanese yen'.
  static const IconData japaneseYen = IconData(
    57601,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'joystick'.
  static const IconData joystick = IconData(58200, fontFamily: 'LucideIcons');

  /// Lucide icon for 'kanban'.
  static const IconData kanban = IconData(58592, fontFamily: 'LucideIcons');

  /// Lucide icon for 'key round'.
  static const IconData keyRound = IconData(58535, fontFamily: 'LucideIcons');

  /// Lucide icon for 'key square'.
  static const IconData keySquare = IconData(58536, fontFamily: 'LucideIcons');

  /// Lucide icon for 'key'.
  static const IconData key = IconData(57602, fontFamily: 'LucideIcons');

  /// Lucide icon for 'keyboard music'.
  static const IconData keyboardMusic = IconData(
    58724,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'keyboard off'.
  static const IconData keyboardOff = IconData(
    58850,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'keyboard'.
  static const IconData keyboard = IconData(57987, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lamp ceiling'.
  static const IconData lampCeiling = IconData(
    58072,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'lamp desk'.
  static const IconData lampDesk = IconData(58073, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lamp floor'.
  static const IconData lampFloor = IconData(58074, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lamp wall down'.
  static const IconData lampWallDown = IconData(
    58075,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'lamp wall up'.
  static const IconData lampWallUp = IconData(58076, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lamp'.
  static const IconData lamp = IconData(58071, fontFamily: 'LucideIcons');

  /// Lucide icon for 'land plot'.
  static const IconData landPlot = IconData(58668, fontFamily: 'LucideIcons');

  /// Lucide icon for 'landmark'.
  static const IconData landmark = IconData(57913, fontFamily: 'LucideIcons');

  /// Lucide icon for 'languages'.
  static const IconData languages = IconData(57603, fontFamily: 'LucideIcons');

  /// Lucide icon for 'laptop minimal check'.
  static const IconData laptopMinimalCheck = IconData(
    58934,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'laptop minimal'.
  static const IconData laptopMinimal = IconData(
    57815,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'laptop'.
  static const IconData laptop = IconData(57804, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lasso select'.
  static const IconData lassoSelect = IconData(
    57806,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'lasso'.
  static const IconData lasso = IconData(57805, fontFamily: 'LucideIcons');

  /// Lucide icon for 'laugh'.
  static const IconData laugh = IconData(58111, fontFamily: 'LucideIcons');

  /// Lucide icon for 'layers 2'.
  static const IconData layers2 = IconData(58670, fontFamily: 'LucideIcons');

  /// Lucide icon for 'layers'.
  static const IconData layers = IconData(58669, fontFamily: 'LucideIcons');

  /// Lucide icon for 'layout dashboard'.
  static const IconData layoutDashboard = IconData(
    57792,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'layout grid'.
  static const IconData layoutGrid = IconData(57604, fontFamily: 'LucideIcons');

  /// Lucide icon for 'layout list'.
  static const IconData layoutList = IconData(57816, fontFamily: 'LucideIcons');

  /// Lucide icon for 'layout panel left'.
  static const IconData layoutPanelLeft = IconData(
    58484,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'layout panel top'.
  static const IconData layoutPanelTop = IconData(
    58485,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'layout template'.
  static const IconData layoutTemplate = IconData(
    57862,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'leaf'.
  static const IconData leaf = IconData(58077, fontFamily: 'LucideIcons');

  /// Lucide icon for 'leafy green'.
  static const IconData leafyGreen = IconData(58483, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lectern'.
  static const IconData lectern = IconData(58861, fontFamily: 'LucideIcons');

  /// Lucide icon for 'letter text'.
  static const IconData letterText = IconData(58889, fontFamily: 'LucideIcons');

  /// Lucide icon for 'library big'.
  static const IconData libraryBig = IconData(58706, fontFamily: 'LucideIcons');

  /// Lucide icon for 'library'.
  static const IconData library = IconData(57605, fontFamily: 'LucideIcons');

  /// Lucide icon for 'life buoy'.
  static const IconData lifeBuoy = IconData(57606, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ligature'.
  static const IconData ligature = IconData(58430, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lightbulb off'.
  static const IconData lightbulbOff = IconData(
    57863,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'lightbulb'.
  static const IconData lightbulb = IconData(57793, fontFamily: 'LucideIcons');

  /// Lucide icon for 'link 2 off'.
  static const IconData link2Off = IconData(57609, fontFamily: 'LucideIcons');

  /// Lucide icon for 'link 2'.
  static const IconData link2 = IconData(57608, fontFamily: 'LucideIcons');

  /// Lucide icon for 'link'.
  static const IconData link = IconData(57607, fontFamily: 'LucideIcons');

  /// Lucide icon for 'linkedin'.
  static const IconData linkedin = IconData(57610, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list check'.
  static const IconData listCheck = IconData(58878, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list checks'.
  static const IconData listChecks = IconData(57807, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list collapse'.
  static const IconData listCollapse = IconData(
    58783,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'list end'.
  static const IconData listEnd = IconData(58078, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list filter plus'.
  static const IconData listFilterPlus = IconData(
    58941,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'list filter'.
  static const IconData listFilter = IconData(58468, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list minus'.
  static const IconData listMinus = IconData(57917, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list music'.
  static const IconData listMusic = IconData(58079, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list ordered'.
  static const IconData listOrdered = IconData(
    57808,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'list plus'.
  static const IconData listPlus = IconData(57918, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list restart'.
  static const IconData listRestart = IconData(
    58454,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'list start'.
  static const IconData listStart = IconData(58080, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list todo'.
  static const IconData listTodo = IconData(58567, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list tree'.
  static const IconData listTree = IconData(58380, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list video'.
  static const IconData listVideo = IconData(58081, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list X'.
  static const IconData listX = IconData(57919, fontFamily: 'LucideIcons');

  /// Lucide icon for 'list'.
  static const IconData list = IconData(57611, fontFamily: 'LucideIcons');

  /// Lucide icon for 'loader circle'.
  static const IconData loaderCircle = IconData(
    57613,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'loader pinwheel'.
  static const IconData loaderPinwheel = IconData(
    58858,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'loader'.
  static const IconData loader = IconData(57612, fontFamily: 'LucideIcons');

  /// Lucide icon for 'locate fixed'.
  static const IconData locateFixed = IconData(
    57818,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'locate off'.
  static const IconData locateOff = IconData(57985, fontFamily: 'LucideIcons');

  /// Lucide icon for 'locate'.
  static const IconData locate = IconData(57817, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lock keyhole open'.
  static const IconData lockKeyholeOpen = IconData(
    58678,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'lock keyhole'.
  static const IconData lockKeyhole = IconData(
    58677,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'lock open'.
  static const IconData lockOpen = IconData(57615, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lock'.
  static const IconData lock = IconData(57614, fontFamily: 'LucideIcons');

  /// Lucide icon for 'log in'.
  static const IconData logIn = IconData(57616, fontFamily: 'LucideIcons');

  /// Lucide icon for 'log out'.
  static const IconData logOut = IconData(57617, fontFamily: 'LucideIcons');

  /// Lucide icon for 'logs'.
  static const IconData logs = IconData(58872, fontFamily: 'LucideIcons');

  /// Lucide icon for 'lollipop'.
  static const IconData lollipop = IconData(58561, fontFamily: 'LucideIcons');

  /// Lucide icon for 'luggage'.
  static const IconData luggage = IconData(58057, fontFamily: 'LucideIcons');

  /// Lucide icon for 'magnet'.
  static const IconData magnet = IconData(58036, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mail check'.
  static const IconData mailCheck = IconData(58212, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mail minus'.
  static const IconData mailMinus = IconData(58213, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mail open'.
  static const IconData mailOpen = IconData(58214, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mail plus'.
  static const IconData mailPlus = IconData(58215, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mail question'.
  static const IconData mailQuestion = IconData(
    58216,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'mail search'.
  static const IconData mailSearch = IconData(58217, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mail warning'.
  static const IconData mailWarning = IconData(
    58218,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'mail X'.
  static const IconData mailX = IconData(58219, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mail'.
  static const IconData mail = IconData(57618, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mailbox'.
  static const IconData mailbox = IconData(58327, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mails'.
  static const IconData mails = IconData(58220, fontFamily: 'LucideIcons');

  /// Lucide icon for 'map pin check inside'.
  static const IconData mapPinCheckInside = IconData(
    58900,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'map pin check'.
  static const IconData mapPinCheck = IconData(
    58899,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'map pin house'.
  static const IconData mapPinHouse = IconData(
    58912,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'map pin minus inside'.
  static const IconData mapPinMinusInside = IconData(
    58902,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'map pin minus'.
  static const IconData mapPinMinus = IconData(
    58901,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'map pin off'.
  static const IconData mapPinOff = IconData(58021, fontFamily: 'LucideIcons');

  /// Lucide icon for 'map pin plus inside'.
  static const IconData mapPinPlusInside = IconData(
    58904,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'map pin plus'.
  static const IconData mapPinPlus = IconData(58903, fontFamily: 'LucideIcons');

  /// Lucide icon for 'map pin X inside'.
  static const IconData mapPinXInside = IconData(
    58906,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'map pin X'.
  static const IconData mapPinX = IconData(58905, fontFamily: 'LucideIcons');

  /// Lucide icon for 'map pin'.
  static const IconData mapPin = IconData(57620, fontFamily: 'LucideIcons');

  /// Lucide icon for 'map pinned'.
  static const IconData mapPinned = IconData(58689, fontFamily: 'LucideIcons');

  /// Lucide icon for 'map plus'.
  static const IconData mapPlus = IconData(58947, fontFamily: 'LucideIcons');

  /// Lucide icon for 'map'.
  static const IconData map = IconData(57619, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mars stroke'.
  static const IconData marsStroke = IconData(58950, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mars'.
  static const IconData mars = IconData(58949, fontFamily: 'LucideIcons');

  /// Lucide icon for 'martini'.
  static const IconData martini = IconData(58082, fontFamily: 'LucideIcons');

  /// Lucide icon for 'maximize 2'.
  static const IconData maximize2 = IconData(57622, fontFamily: 'LucideIcons');

  /// Lucide icon for 'maximize'.
  static const IconData maximize = IconData(57621, fontFamily: 'LucideIcons');

  /// Lucide icon for 'medal'.
  static const IconData medal = IconData(58226, fontFamily: 'LucideIcons');

  /// Lucide icon for 'megaphone off'.
  static const IconData megaphoneOff = IconData(
    58227,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'megaphone'.
  static const IconData megaphone = IconData(57908, fontFamily: 'LucideIcons');

  /// Lucide icon for 'meh'.
  static const IconData meh = IconData(57623, fontFamily: 'LucideIcons');

  /// Lucide icon for 'memory stick'.
  static const IconData memoryStick = IconData(
    58441,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'menu'.
  static const IconData menu = IconData(57624, fontFamily: 'LucideIcons');

  /// Lucide icon for 'merge'.
  static const IconData merge = IconData(58435, fontFamily: 'LucideIcons');

  /// Lucide icon for 'message circle code'.
  static const IconData messageCircleCode = IconData(
    58726,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle dashed'.
  static const IconData messageCircleDashed = IconData(
    58727,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle heart'.
  static const IconData messageCircleHeart = IconData(
    58728,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle more'.
  static const IconData messageCircleMore = IconData(
    58729,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle off'.
  static const IconData messageCircleOff = IconData(
    58730,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle plus'.
  static const IconData messageCirclePlus = IconData(
    58731,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle question'.
  static const IconData messageCircleQuestion = IconData(
    58732,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle reply'.
  static const IconData messageCircleReply = IconData(
    58733,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle warning'.
  static const IconData messageCircleWarning = IconData(
    58734,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle X'.
  static const IconData messageCircleX = IconData(
    58735,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message circle'.
  static const IconData messageCircle = IconData(
    57625,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square code'.
  static const IconData messageSquareCode = IconData(
    58736,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square dashed'.
  static const IconData messageSquareDashed = IconData(
    58383,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square diff'.
  static const IconData messageSquareDiff = IconData(
    58737,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square dot'.
  static const IconData messageSquareDot = IconData(
    58738,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square heart'.
  static const IconData messageSquareHeart = IconData(
    58739,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square lock'.
  static const IconData messageSquareLock = IconData(
    58928,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square more'.
  static const IconData messageSquareMore = IconData(
    58740,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square off'.
  static const IconData messageSquareOff = IconData(
    58741,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square plus'.
  static const IconData messageSquarePlus = IconData(
    58384,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square quote'.
  static const IconData messageSquareQuote = IconData(
    58742,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square reply'.
  static const IconData messageSquareReply = IconData(
    58743,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square share'.
  static const IconData messageSquareShare = IconData(
    58744,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square text'.
  static const IconData messageSquareText = IconData(
    58745,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square warning'.
  static const IconData messageSquareWarning = IconData(
    58746,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square X'.
  static const IconData messageSquareX = IconData(
    58747,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'message square'.
  static const IconData messageSquare = IconData(
    57626,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'messages square'.
  static const IconData messagesSquare = IconData(
    58385,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'mic off'.
  static const IconData micOff = IconData(57628, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mic vocal'.
  static const IconData micVocal = IconData(58188, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mic'.
  static const IconData mic = IconData(57627, fontFamily: 'LucideIcons');

  /// Lucide icon for 'microchip'.
  static const IconData microchip = IconData(58910, fontFamily: 'LucideIcons');

  /// Lucide icon for 'microscope'.
  static const IconData microscope = IconData(58083, fontFamily: 'LucideIcons');

  /// Lucide icon for 'microwave'.
  static const IconData microwave = IconData(58237, fontFamily: 'LucideIcons');

  /// Lucide icon for 'milestone'.
  static const IconData milestone = IconData(58007, fontFamily: 'LucideIcons');

  /// Lucide icon for 'milk off'.
  static const IconData milkOff = IconData(58269, fontFamily: 'LucideIcons');

  /// Lucide icon for 'milk'.
  static const IconData milk = IconData(58268, fontFamily: 'LucideIcons');

  /// Lucide icon for 'minimize 2'.
  static const IconData minimize2 = IconData(57630, fontFamily: 'LucideIcons');

  /// Lucide icon for 'minimize'.
  static const IconData minimize = IconData(57629, fontFamily: 'LucideIcons');

  /// Lucide icon for 'minus'.
  static const IconData minus = IconData(57631, fontFamily: 'LucideIcons');

  /// Lucide icon for 'monitor check'.
  static const IconData monitorCheck = IconData(
    58502,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'monitor cog'.
  static const IconData monitorCog = IconData(58887, fontFamily: 'LucideIcons');

  /// Lucide icon for 'monitor dot'.
  static const IconData monitorDot = IconData(58503, fontFamily: 'LucideIcons');

  /// Lucide icon for 'monitor down'.
  static const IconData monitorDown = IconData(
    58405,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'monitor off'.
  static const IconData monitorOff = IconData(57819, fontFamily: 'LucideIcons');

  /// Lucide icon for 'monitor pause'.
  static const IconData monitorPause = IconData(
    58504,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'monitor play'.
  static const IconData monitorPlay = IconData(
    58505,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'monitor smartphone'.
  static const IconData monitorSmartphone = IconData(
    58277,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'monitor speaker'.
  static const IconData monitorSpeaker = IconData(
    57871,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'monitor stop'.
  static const IconData monitorStop = IconData(
    58506,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'monitor up'.
  static const IconData monitorUp = IconData(58406, fontFamily: 'LucideIcons');

  /// Lucide icon for 'monitor X'.
  static const IconData monitorX = IconData(58507, fontFamily: 'LucideIcons');

  /// Lucide icon for 'monitor'.
  static const IconData monitor = IconData(57632, fontFamily: 'LucideIcons');

  /// Lucide icon for 'moon star'.
  static const IconData moonStar = IconData(58388, fontFamily: 'LucideIcons');

  /// Lucide icon for 'moon'.
  static const IconData moon = IconData(57633, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mountain snow'.
  static const IconData mountainSnow = IconData(
    57905,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'mountain'.
  static const IconData mountain = IconData(57904, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mouse off'.
  static const IconData mouseOff = IconData(58847, fontFamily: 'LucideIcons');

  /// Lucide icon for 'mouse pointer 2'.
  static const IconData mousePointer2 = IconData(
    57794,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'mouse pointer ban'.
  static const IconData mousePointerBan = IconData(
    58859,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'mouse pointer click'.
  static const IconData mousePointerClick = IconData(
    57635,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'mouse pointer'.
  static const IconData mousePointer = IconData(
    57634,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'mouse'.
  static const IconData mouse = IconData(57997, fontFamily: 'LucideIcons');

  /// Lucide icon for 'move 3d'.
  static const IconData move3d = IconData(58084, fontFamily: 'LucideIcons');

  /// Lucide icon for 'move diagonal 2'.
  static const IconData moveDiagonal2 = IconData(
    57796,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'move diagonal'.
  static const IconData moveDiagonal = IconData(
    57795,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'move down left'.
  static const IconData moveDownLeft = IconData(
    58513,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'move down right'.
  static const IconData moveDownRight = IconData(
    58514,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'move down'.
  static const IconData moveDown = IconData(58512, fontFamily: 'LucideIcons');

  /// Lucide icon for 'move horizontal'.
  static const IconData moveHorizontal = IconData(
    57797,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'move left'.
  static const IconData moveLeft = IconData(58515, fontFamily: 'LucideIcons');

  /// Lucide icon for 'move right'.
  static const IconData moveRight = IconData(58516, fontFamily: 'LucideIcons');

  /// Lucide icon for 'move up left'.
  static const IconData moveUpLeft = IconData(58518, fontFamily: 'LucideIcons');

  /// Lucide icon for 'move up right'.
  static const IconData moveUpRight = IconData(
    58519,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'move up'.
  static const IconData moveUp = IconData(58517, fontFamily: 'LucideIcons');

  /// Lucide icon for 'move vertical'.
  static const IconData moveVertical = IconData(
    57798,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'move'.
  static const IconData move = IconData(57636, fontFamily: 'LucideIcons');

  /// Lucide icon for 'music 2'.
  static const IconData music2 = IconData(58189, fontFamily: 'LucideIcons');

  /// Lucide icon for 'music 3'.
  static const IconData music3 = IconData(58190, fontFamily: 'LucideIcons');

  /// Lucide icon for 'music 4'.
  static const IconData music4 = IconData(58191, fontFamily: 'LucideIcons');

  /// Lucide icon for 'music'.
  static const IconData music = IconData(57637, fontFamily: 'LucideIcons');

  /// Lucide icon for 'navigation 2 off'.
  static const IconData navigation2Off = IconData(
    58022,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'navigation 2'.
  static const IconData navigation2 = IconData(
    57639,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'navigation off'.
  static const IconData navigationOff = IconData(
    58023,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'navigation'.
  static const IconData navigation = IconData(57638, fontFamily: 'LucideIcons');

  /// Lucide icon for 'network'.
  static const IconData network = IconData(57640, fontFamily: 'LucideIcons');

  /// Lucide icon for 'newspaper'.
  static const IconData newspaper = IconData(58187, fontFamily: 'LucideIcons');

  /// Lucide icon for 'nfc'.
  static const IconData nfc = IconData(58310, fontFamily: 'LucideIcons');

  /// Lucide icon for 'non binary'.
  static const IconData nonBinary = IconData(58951, fontFamily: 'LucideIcons');

  /// Lucide icon for 'notebook pen'.
  static const IconData notebookPen = IconData(
    58778,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'notebook tabs'.
  static const IconData notebookTabs = IconData(
    58779,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'notebook text'.
  static const IconData notebookText = IconData(
    58780,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'notebook'.
  static const IconData notebook = IconData(58777, fontFamily: 'LucideIcons');

  /// Lucide icon for 'notepad text dashed'.
  static const IconData notepadTextDashed = IconData(
    58782,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'notepad text'.
  static const IconData notepadText = IconData(
    58781,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'nut off'.
  static const IconData nutOff = IconData(58271, fontFamily: 'LucideIcons');

  /// Lucide icon for 'nut'.
  static const IconData nut = IconData(58270, fontFamily: 'LucideIcons');

  /// Lucide icon for 'octagon alert'.
  static const IconData octagonAlert = IconData(
    57642,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'octagon minus'.
  static const IconData octagonMinus = IconData(
    58923,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'octagon pause'.
  static const IconData octagonPause = IconData(
    57882,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'octagon X'.
  static const IconData octagonX = IconData(57643, fontFamily: 'LucideIcons');

  /// Lucide icon for 'octagon'.
  static const IconData octagon = IconData(57641, fontFamily: 'LucideIcons');

  /// Lucide icon for 'omega'.
  static const IconData omega = IconData(58909, fontFamily: 'LucideIcons');

  /// Lucide icon for 'option'.
  static const IconData option = IconData(57847, fontFamily: 'LucideIcons');

  /// Lucide icon for 'orbit'.
  static const IconData orbit = IconData(58346, fontFamily: 'LucideIcons');

  /// Lucide icon for 'origami'.
  static const IconData origami = IconData(58855, fontFamily: 'LucideIcons');

  /// Lucide icon for 'package 2'.
  static const IconData package2 = IconData(58179, fontFamily: 'LucideIcons');

  /// Lucide icon for 'package check'.
  static const IconData packageCheck = IconData(
    57957,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'package minus'.
  static const IconData packageMinus = IconData(
    57958,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'package open'.
  static const IconData packageOpen = IconData(
    58059,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'package plus'.
  static const IconData packagePlus = IconData(
    57959,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'package search'.
  static const IconData packageSearch = IconData(
    57960,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'package X'.
  static const IconData packageX = IconData(57961, fontFamily: 'LucideIcons');

  /// Lucide icon for 'package'.
  static const IconData package = IconData(57644, fontFamily: 'LucideIcons');

  /// Lucide icon for 'paint bucket'.
  static const IconData paintBucket = IconData(
    58085,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'paint roller'.
  static const IconData paintRoller = IconData(
    58786,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'paintbrush vertical'.
  static const IconData paintbrushVertical = IconData(
    58087,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'paintbrush'.
  static const IconData paintbrush = IconData(58086, fontFamily: 'LucideIcons');

  /// Lucide icon for 'palette'.
  static const IconData palette = IconData(57820, fontFamily: 'LucideIcons');

  /// Lucide icon for 'panel bottom close'.
  static const IconData panelBottomClose = IconData(
    58417,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel bottom dashed'.
  static const IconData panelBottomDashed = IconData(
    58418,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel bottom open'.
  static const IconData panelBottomOpen = IconData(
    58419,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel bottom'.
  static const IconData panelBottom = IconData(
    58416,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel left close'.
  static const IconData panelLeftClose = IconData(
    57883,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel left dashed'.
  static const IconData panelLeftDashed = IconData(
    58420,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel left open'.
  static const IconData panelLeftOpen = IconData(
    57884,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel left'.
  static const IconData panelLeft = IconData(57645, fontFamily: 'LucideIcons');

  /// Lucide icon for 'panel right close'.
  static const IconData panelRightClose = IconData(
    58422,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel right dashed'.
  static const IconData panelRightDashed = IconData(
    58423,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel right open'.
  static const IconData panelRightOpen = IconData(
    58424,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel right'.
  static const IconData panelRight = IconData(58421, fontFamily: 'LucideIcons');

  /// Lucide icon for 'panel top close'.
  static const IconData panelTopClose = IconData(
    58426,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel top dashed'.
  static const IconData panelTopDashed = IconData(
    58427,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel top open'.
  static const IconData panelTopOpen = IconData(
    58428,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panel top'.
  static const IconData panelTop = IconData(58425, fontFamily: 'LucideIcons');

  /// Lucide icon for 'panels left bottom'.
  static const IconData panelsLeftBottom = IconData(
    57646,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panels right bottom'.
  static const IconData panelsRightBottom = IconData(
    58764,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'panels top left'.
  static const IconData panelsTopLeft = IconData(
    57647,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'paperclip'.
  static const IconData paperclip = IconData(57648, fontFamily: 'LucideIcons');

  /// Lucide icon for 'parentheses'.
  static const IconData parentheses = IconData(
    58440,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'parking meter'.
  static const IconData parkingMeter = IconData(
    58628,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'party popper'.
  static const IconData partyPopper = IconData(
    58182,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'pause'.
  static const IconData pause = IconData(57649, fontFamily: 'LucideIcons');

  /// Lucide icon for 'paw print'.
  static const IconData pawPrint = IconData(58617, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pc case'.
  static const IconData pcCase = IconData(58442, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pen line'.
  static const IconData penLine = IconData(57651, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pen off'.
  static const IconData penOff = IconData(58866, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pen tool'.
  static const IconData penTool = IconData(57652, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pen'.
  static const IconData pen = IconData(57650, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pencil line'.
  static const IconData pencilLine = IconData(58612, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pencil off'.
  static const IconData pencilOff = IconData(58867, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pencil ruler'.
  static const IconData pencilRuler = IconData(
    58613,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'pencil'.
  static const IconData pencil = IconData(57848, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pentagon'.
  static const IconData pentagon = IconData(58671, fontFamily: 'LucideIcons');

  /// Lucide icon for 'percent'.
  static const IconData percent = IconData(57653, fontFamily: 'LucideIcons');

  /// Lucide icon for 'person standing'.
  static const IconData personStanding = IconData(
    57885,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'philippine peso'.
  static const IconData philippinePeso = IconData(
    58888,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'phone call'.
  static const IconData phoneCall = IconData(57655, fontFamily: 'LucideIcons');

  /// Lucide icon for 'phone forwarded'.
  static const IconData phoneForwarded = IconData(
    57656,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'phone incoming'.
  static const IconData phoneIncoming = IconData(
    57657,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'phone missed'.
  static const IconData phoneMissed = IconData(
    57658,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'phone off'.
  static const IconData phoneOff = IconData(57659, fontFamily: 'LucideIcons');

  /// Lucide icon for 'phone outgoing'.
  static const IconData phoneOutgoing = IconData(
    57660,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'phone'.
  static const IconData phone = IconData(57654, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pi'.
  static const IconData pi = IconData(58486, fontFamily: 'LucideIcons');

  /// Lucide icon for 'piano'.
  static const IconData piano = IconData(58725, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pickaxe'.
  static const IconData pickaxe = IconData(58826, fontFamily: 'LucideIcons');

  /// Lucide icon for 'picture in picture 2'.
  static const IconData pictureInPicture2 = IconData(
    58290,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'picture in picture'.
  static const IconData pictureInPicture = IconData(
    58289,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'piggy bank'.
  static const IconData piggyBank = IconData(57661, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pilcrow left'.
  static const IconData pilcrowLeft = IconData(
    58848,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'pilcrow right'.
  static const IconData pilcrowRight = IconData(
    58849,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'pilcrow'.
  static const IconData pilcrow = IconData(58278, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pill bottle'.
  static const IconData pillBottle = IconData(58862, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pill'.
  static const IconData pill = IconData(58304, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pin off'.
  static const IconData pinOff = IconData(58037, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pin'.
  static const IconData pin = IconData(57944, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pipette'.
  static const IconData pipette = IconData(57662, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pizza'.
  static const IconData pizza = IconData(58199, fontFamily: 'LucideIcons');

  /// Lucide icon for 'plane landing'.
  static const IconData planeLanding = IconData(
    58320,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'plane takeoff'.
  static const IconData planeTakeoff = IconData(
    58321,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'plane'.
  static const IconData plane = IconData(57821, fontFamily: 'LucideIcons');

  /// Lucide icon for 'play'.
  static const IconData play = IconData(57663, fontFamily: 'LucideIcons');

  /// Lucide icon for 'plug 2'.
  static const IconData plug2 = IconData(58243, fontFamily: 'LucideIcons');

  /// Lucide icon for 'plug zap'.
  static const IconData plugZap = IconData(58464, fontFamily: 'LucideIcons');

  /// Lucide icon for 'plug'.
  static const IconData plug = IconData(58242, fontFamily: 'LucideIcons');

  /// Lucide icon for 'plus'.
  static const IconData plus = IconData(57664, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pocket knife'.
  static const IconData pocketKnife = IconData(
    58532,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'pocket'.
  static const IconData pocket = IconData(57665, fontFamily: 'LucideIcons');

  /// Lucide icon for 'podcast'.
  static const IconData podcast = IconData(57849, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pointer off'.
  static const IconData pointerOff = IconData(58755, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pointer'.
  static const IconData pointer = IconData(57831, fontFamily: 'LucideIcons');

  /// Lucide icon for 'popcorn'.
  static const IconData popcorn = IconData(58562, fontFamily: 'LucideIcons');

  /// Lucide icon for 'popsicle'.
  static const IconData popsicle = IconData(58563, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pound sterling'.
  static const IconData poundSterling = IconData(
    57666,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'power off'.
  static const IconData powerOff = IconData(57864, fontFamily: 'LucideIcons');

  /// Lucide icon for 'power'.
  static const IconData power = IconData(57667, fontFamily: 'LucideIcons');

  /// Lucide icon for 'presentation'.
  static const IconData presentation = IconData(
    58546,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'printer check'.
  static const IconData printerCheck = IconData(
    58873,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'printer'.
  static const IconData printer = IconData(57668, fontFamily: 'LucideIcons');

  /// Lucide icon for 'projector'.
  static const IconData projector = IconData(58547, fontFamily: 'LucideIcons');

  /// Lucide icon for 'proportions'.
  static const IconData proportions = IconData(
    58835,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'puzzle'.
  static const IconData puzzle = IconData(58011, fontFamily: 'LucideIcons');

  /// Lucide icon for 'pyramid'.
  static const IconData pyramid = IconData(58672, fontFamily: 'LucideIcons');

  /// Lucide icon for 'qr code'.
  static const IconData qrCode = IconData(57822, fontFamily: 'LucideIcons');

  /// Lucide icon for 'quote'.
  static const IconData quote = IconData(57912, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rabbit'.
  static const IconData rabbit = IconData(58618, fontFamily: 'LucideIcons');

  /// Lucide icon for 'radar'.
  static const IconData radar = IconData(58523, fontFamily: 'LucideIcons');

  /// Lucide icon for 'radiation'.
  static const IconData radiation = IconData(58438, fontFamily: 'LucideIcons');

  /// Lucide icon for 'radical'.
  static const IconData radical = IconData(58822, fontFamily: 'LucideIcons');

  /// Lucide icon for 'radio receiver'.
  static const IconData radioReceiver = IconData(
    57850,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'radio tower'.
  static const IconData radioTower = IconData(58376, fontFamily: 'LucideIcons');

  /// Lucide icon for 'radio'.
  static const IconData radio = IconData(57669, fontFamily: 'LucideIcons');

  /// Lucide icon for 'radius'.
  static const IconData radius = IconData(58673, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rail symbol'.
  static const IconData railSymbol = IconData(58629, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rainbow'.
  static const IconData rainbow = IconData(58566, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rat'.
  static const IconData rat = IconData(58351, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ratio'.
  static const IconData ratio = IconData(58604, fontFamily: 'LucideIcons');

  /// Lucide icon for 'receipt cent'.
  static const IconData receiptCent = IconData(
    58793,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'receipt euro'.
  static const IconData receiptEuro = IconData(
    58794,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'receipt indian rupee'.
  static const IconData receiptIndianRupee = IconData(
    58795,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'receipt japanese yen'.
  static const IconData receiptJapaneseYen = IconData(
    58796,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'receipt pound sterling'.
  static const IconData receiptPoundSterling = IconData(
    58797,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'receipt russian ruble'.
  static const IconData receiptRussianRuble = IconData(
    58798,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'receipt swiss franc'.
  static const IconData receiptSwissFranc = IconData(
    58799,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'receipt text'.
  static const IconData receiptText = IconData(
    58800,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'receipt'.
  static const IconData receipt = IconData(58326, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rectangle ellipsis'.
  static const IconData rectangleEllipsis = IconData(
    57886,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'rectangle horizontal'.
  static const IconData rectangleHorizontal = IconData(
    58233,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'rectangle vertical'.
  static const IconData rectangleVertical = IconData(
    58234,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'recycle'.
  static const IconData recycle = IconData(58088, fontFamily: 'LucideIcons');

  /// Lucide icon for 'redo 2'.
  static const IconData redo2 = IconData(58015, fontFamily: 'LucideIcons');

  /// Lucide icon for 'redo dot'.
  static const IconData redoDot = IconData(58452, fontFamily: 'LucideIcons');

  /// Lucide icon for 'redo'.
  static const IconData redo = IconData(57670, fontFamily: 'LucideIcons');

  /// Lucide icon for 'refresh ccw dot'.
  static const IconData refreshCcwDot = IconData(
    58550,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'refresh ccw'.
  static const IconData refreshCcw = IconData(57671, fontFamily: 'LucideIcons');

  /// Lucide icon for 'refresh cw off'.
  static const IconData refreshCwOff = IconData(
    58524,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'refresh cw'.
  static const IconData refreshCw = IconData(57672, fontFamily: 'LucideIcons');

  /// Lucide icon for 'refrigerator'.
  static const IconData refrigerator = IconData(
    58238,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'regex'.
  static const IconData regex = IconData(57851, fontFamily: 'LucideIcons');

  /// Lucide icon for 'remove formatting'.
  static const IconData removeFormatting = IconData(
    58294,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'repeat 1'.
  static const IconData repeat1 = IconData(57852, fontFamily: 'LucideIcons');

  /// Lucide icon for 'repeat 2'.
  static const IconData repeat2 = IconData(58389, fontFamily: 'LucideIcons');

  /// Lucide icon for 'repeat'.
  static const IconData repeat = IconData(57673, fontFamily: 'LucideIcons');

  /// Lucide icon for 'replace all'.
  static const IconData replaceAll = IconData(58335, fontFamily: 'LucideIcons');

  /// Lucide icon for 'replace'.
  static const IconData replace = IconData(58334, fontFamily: 'LucideIcons');

  /// Lucide icon for 'reply all'.
  static const IconData replyAll = IconData(57898, fontFamily: 'LucideIcons');

  /// Lucide icon for 'reply'.
  static const IconData reply = IconData(57897, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rewind'.
  static const IconData rewind = IconData(57674, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ribbon'.
  static const IconData ribbon = IconData(58716, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rocket'.
  static const IconData rocket = IconData(57989, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rocking chair'.
  static const IconData rockingChair = IconData(
    57906,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'roller coaster'.
  static const IconData rollerCoaster = IconData(
    58500,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'rotate 3d'.
  static const IconData rotate3d = IconData(58089, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rotate ccw square'.
  static const IconData rotateCcwSquare = IconData(
    58836,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'rotate ccw'.
  static const IconData rotateCcw = IconData(57675, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rotate cw square'.
  static const IconData rotateCwSquare = IconData(
    58837,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'rotate cw'.
  static const IconData rotateCw = IconData(57676, fontFamily: 'LucideIcons');

  /// Lucide icon for 'route off'.
  static const IconData routeOff = IconData(58691, fontFamily: 'LucideIcons');

  /// Lucide icon for 'route'.
  static const IconData route = IconData(58690, fontFamily: 'LucideIcons');

  /// Lucide icon for 'router'.
  static const IconData router = IconData(58306, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rows 2'.
  static const IconData rows2 = IconData(58429, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rows 3'.
  static const IconData rows3 = IconData(58766, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rows 4'.
  static const IconData rows4 = IconData(58767, fontFamily: 'LucideIcons');

  /// Lucide icon for 'rss'.
  static const IconData rss = IconData(57677, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ruler'.
  static const IconData ruler = IconData(57678, fontFamily: 'LucideIcons');

  /// Lucide icon for 'russian ruble'.
  static const IconData russianRuble = IconData(
    57679,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'sailboat'.
  static const IconData sailboat = IconData(58241, fontFamily: 'LucideIcons');

  /// Lucide icon for 'salad'.
  static const IconData salad = IconData(58283, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sandwich'.
  static const IconData sandwich = IconData(58284, fontFamily: 'LucideIcons');

  /// Lucide icon for 'satellite dish'.
  static const IconData satelliteDish = IconData(
    58444,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'satellite'.
  static const IconData satellite = IconData(58443, fontFamily: 'LucideIcons');

  /// Lucide icon for 'save all'.
  static const IconData saveAll = IconData(58387, fontFamily: 'LucideIcons');

  /// Lucide icon for 'save off'.
  static const IconData saveOff = IconData(58871, fontFamily: 'LucideIcons');

  /// Lucide icon for 'save'.
  static const IconData save = IconData(57680, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scale 3d'.
  static const IconData scale3d = IconData(58090, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scale'.
  static const IconData scale = IconData(57873, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scaling'.
  static const IconData scaling = IconData(58091, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scan barcode'.
  static const IconData scanBarcode = IconData(
    58681,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'scan eye'.
  static const IconData scanEye = IconData(58682, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scan face'.
  static const IconData scanFace = IconData(58228, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scan heart'.
  static const IconData scanHeart = IconData(58942, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scan line'.
  static const IconData scanLine = IconData(57943, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scan qr code'.
  static const IconData scanQrCode = IconData(58874, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scan search'.
  static const IconData scanSearch = IconData(58683, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scan text'.
  static const IconData scanText = IconData(58684, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scan'.
  static const IconData scan = IconData(57942, fontFamily: 'LucideIcons');

  /// Lucide icon for 'school'.
  static const IconData school = IconData(58342, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scissors line dashed'.
  static const IconData scissorsLineDashed = IconData(
    58605,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'scissors'.
  static const IconData scissors = IconData(57681, fontFamily: 'LucideIcons');

  /// Lucide icon for 'screen share off'.
  static const IconData screenShareOff = IconData(
    57683,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'screen share'.
  static const IconData screenShare = IconData(
    57682,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'scroll text'.
  static const IconData scrollText = IconData(58467, fontFamily: 'LucideIcons');

  /// Lucide icon for 'scroll'.
  static const IconData scroll = IconData(58092, fontFamily: 'LucideIcons');

  /// Lucide icon for 'search check'.
  static const IconData searchCheck = IconData(
    58542,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'search code'.
  static const IconData searchCode = IconData(58543, fontFamily: 'LucideIcons');

  /// Lucide icon for 'search slash'.
  static const IconData searchSlash = IconData(
    58544,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'search X'.
  static const IconData searchX = IconData(58545, fontFamily: 'LucideIcons');

  /// Lucide icon for 'search'.
  static const IconData search = IconData(57684, fontFamily: 'LucideIcons');

  /// Lucide icon for 'section'.
  static const IconData section = IconData(58860, fontFamily: 'LucideIcons');

  /// Lucide icon for 'send horizontal'.
  static const IconData sendHorizontal = IconData(
    58614,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'send to back'.
  static const IconData sendToBack = IconData(58615, fontFamily: 'LucideIcons');

  /// Lucide icon for 'send'.
  static const IconData send = IconData(57685, fontFamily: 'LucideIcons');

  /// Lucide icon for 'separator horizontal'.
  static const IconData separatorHorizontal = IconData(
    57799,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'separator vertical'.
  static const IconData separatorVertical = IconData(
    57800,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'server cog'.
  static const IconData serverCog = IconData(58180, fontFamily: 'LucideIcons');

  /// Lucide icon for 'server crash'.
  static const IconData serverCrash = IconData(
    57832,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'server off'.
  static const IconData serverOff = IconData(57833, fontFamily: 'LucideIcons');

  /// Lucide icon for 'server'.
  static const IconData server = IconData(57686, fontFamily: 'LucideIcons');

  /// Lucide icon for 'settings 2'.
  static const IconData settings2 = IconData(57924, fontFamily: 'LucideIcons');

  /// Lucide icon for 'settings'.
  static const IconData settings = IconData(57687, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shapes'.
  static const IconData shapes = IconData(58551, fontFamily: 'LucideIcons');

  /// Lucide icon for 'share 2'.
  static const IconData share2 = IconData(57689, fontFamily: 'LucideIcons');

  /// Lucide icon for 'share'.
  static const IconData share = IconData(57688, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sheet'.
  static const IconData sheet = IconData(57690, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shell'.
  static const IconData shell = IconData(58619, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shield alert'.
  static const IconData shieldAlert = IconData(
    57853,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'shield ban'.
  static const IconData shieldBan = IconData(57692, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shield check'.
  static const IconData shieldCheck = IconData(
    57854,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'shield ellipsis'.
  static const IconData shieldEllipsis = IconData(
    58650,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'shield half'.
  static const IconData shieldHalf = IconData(58651, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shield minus'.
  static const IconData shieldMinus = IconData(
    58652,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'shield off'.
  static const IconData shieldOff = IconData(57693, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shield plus'.
  static const IconData shieldPlus = IconData(58653, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shield question'.
  static const IconData shieldQuestion = IconData(
    58386,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'shield user'.
  static const IconData shieldUser = IconData(58955, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shield X'.
  static const IconData shieldX = IconData(57855, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shield'.
  static const IconData shield = IconData(57691, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ship wheel'.
  static const IconData shipWheel = IconData(58630, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ship'.
  static const IconData ship = IconData(58301, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shirt'.
  static const IconData shirt = IconData(57801, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shopping bag'.
  static const IconData shoppingBag = IconData(
    57694,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'shopping basket'.
  static const IconData shoppingBasket = IconData(
    58606,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'shopping cart'.
  static const IconData shoppingCart = IconData(
    57695,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'shovel'.
  static const IconData shovel = IconData(57696, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shower head'.
  static const IconData showerHead = IconData(58239, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shrink'.
  static const IconData shrink = IconData(57887, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shrub'.
  static const IconData shrub = IconData(58093, fontFamily: 'LucideIcons');

  /// Lucide icon for 'shuffle'.
  static const IconData shuffle = IconData(57697, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sigma'.
  static const IconData sigma = IconData(57856, fontFamily: 'LucideIcons');

  /// Lucide icon for 'signal high'.
  static const IconData signalHigh = IconData(57951, fontFamily: 'LucideIcons');

  /// Lucide icon for 'signal low'.
  static const IconData signalLow = IconData(57952, fontFamily: 'LucideIcons');

  /// Lucide icon for 'signal medium'.
  static const IconData signalMedium = IconData(
    57953,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'signal zero'.
  static const IconData signalZero = IconData(57954, fontFamily: 'LucideIcons');

  /// Lucide icon for 'signal'.
  static const IconData signal = IconData(57950, fontFamily: 'LucideIcons');

  /// Lucide icon for 'signature'.
  static const IconData signature = IconData(58870, fontFamily: 'LucideIcons');

  /// Lucide icon for 'signpost big'.
  static const IconData signpostBig = IconData(
    58693,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'signpost'.
  static const IconData signpost = IconData(58692, fontFamily: 'LucideIcons');

  /// Lucide icon for 'siren'.
  static const IconData siren = IconData(58094, fontFamily: 'LucideIcons');

  /// Lucide icon for 'skip back'.
  static const IconData skipBack = IconData(57698, fontFamily: 'LucideIcons');

  /// Lucide icon for 'skip forward'.
  static const IconData skipForward = IconData(
    57699,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'skull'.
  static const IconData skull = IconData(57888, fontFamily: 'LucideIcons');

  /// Lucide icon for 'slack'.
  static const IconData slack = IconData(57700, fontFamily: 'LucideIcons');

  /// Lucide icon for 'slash'.
  static const IconData slash = IconData(58657, fontFamily: 'LucideIcons');

  /// Lucide icon for 'slice'.
  static const IconData slice = IconData(58095, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sliders horizontal'.
  static const IconData slidersHorizontal = IconData(
    58009,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'sliders vertical'.
  static const IconData slidersVertical = IconData(
    57701,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'smartphone charging'.
  static const IconData smartphoneCharging = IconData(
    57901,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'smartphone nfc'.
  static const IconData smartphoneNfc = IconData(
    58311,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'smartphone'.
  static const IconData smartphone = IconData(57702, fontFamily: 'LucideIcons');

  /// Lucide icon for 'smile plus'.
  static const IconData smilePlus = IconData(58112, fontFamily: 'LucideIcons');

  /// Lucide icon for 'smile'.
  static const IconData smile = IconData(57703, fontFamily: 'LucideIcons');

  /// Lucide icon for 'snail'.
  static const IconData snail = IconData(58620, fontFamily: 'LucideIcons');

  /// Lucide icon for 'snowflake'.
  static const IconData snowflake = IconData(57704, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sofa'.
  static const IconData sofa = IconData(58051, fontFamily: 'LucideIcons');

  /// Lucide icon for 'soup'.
  static const IconData soup = IconData(58285, fontFamily: 'LucideIcons');

  /// Lucide icon for 'space'.
  static const IconData space = IconData(58336, fontFamily: 'LucideIcons');

  /// Lucide icon for 'spade'.
  static const IconData spade = IconData(58525, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sparkle'.
  static const IconData sparkle = IconData(58498, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sparkles'.
  static const IconData sparkles = IconData(58390, fontFamily: 'LucideIcons');

  /// Lucide icon for 'speaker'.
  static const IconData speaker = IconData(57705, fontFamily: 'LucideIcons');

  /// Lucide icon for 'speech'.
  static const IconData speech = IconData(58658, fontFamily: 'LucideIcons');

  /// Lucide icon for 'spell check 2'.
  static const IconData spellCheck2 = IconData(
    58527,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'spell check'.
  static const IconData spellCheck = IconData(58526, fontFamily: 'LucideIcons');

  /// Lucide icon for 'spline'.
  static const IconData spline = IconData(58254, fontFamily: 'LucideIcons');

  /// Lucide icon for 'split'.
  static const IconData split = IconData(58436, fontFamily: 'LucideIcons');

  /// Lucide icon for 'spray can'.
  static const IconData sprayCan = IconData(58521, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sprout'.
  static const IconData sprout = IconData(57834, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square activity'.
  static const IconData squareActivity = IconData(
    58552,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow down left'.
  static const IconData squareArrowDownLeft = IconData(
    58553,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow down right'.
  static const IconData squareArrowDownRight = IconData(
    58554,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow down'.
  static const IconData squareArrowDown = IconData(
    58411,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow left'.
  static const IconData squareArrowLeft = IconData(
    58412,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow out down left'.
  static const IconData squareArrowOutDownLeft = IconData(
    58789,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow out down right'.
  static const IconData squareArrowOutDownRight = IconData(
    58790,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow out up left'.
  static const IconData squareArrowOutUpLeft = IconData(
    58791,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow out up right'.
  static const IconData squareArrowOutUpRight = IconData(
    58792,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow right'.
  static const IconData squareArrowRight = IconData(
    58413,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow up left'.
  static const IconData squareArrowUpLeft = IconData(
    58555,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow up right'.
  static const IconData squareArrowUpRight = IconData(
    58556,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square arrow up'.
  static const IconData squareArrowUp = IconData(
    58414,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square asterisk'.
  static const IconData squareAsterisk = IconData(
    57707,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square bottom dashed scissors'.
  static const IconData squareBottomDashedScissors = IconData(
    58607,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square chart gantt'.
  static const IconData squareChartGantt = IconData(
    57708,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square check big'.
  static const IconData squareCheckBig = IconData(
    57709,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square check'.
  static const IconData squareCheck = IconData(
    58717,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square chevron down'.
  static const IconData squareChevronDown = IconData(
    58322,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square chevron left'.
  static const IconData squareChevronLeft = IconData(
    58323,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square chevron right'.
  static const IconData squareChevronRight = IconData(
    58324,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square chevron up'.
  static const IconData squareChevronUp = IconData(
    58325,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square code'.
  static const IconData squareCode = IconData(57710, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square dashed bottom code'.
  static const IconData squareDashedBottomCode = IconData(
    58565,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square dashed bottom'.
  static const IconData squareDashedBottom = IconData(
    58564,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square dashed kanban'.
  static const IconData squareDashedKanban = IconData(
    57711,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square dashed mouse pointer'.
  static const IconData squareDashedMousePointer = IconData(
    58637,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square dashed'.
  static const IconData squareDashed = IconData(
    57802,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square divide'.
  static const IconData squareDivide = IconData(
    57712,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square dot'.
  static const IconData squareDot = IconData(57713, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square equal'.
  static const IconData squareEqual = IconData(
    57714,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square function'.
  static const IconData squareFunction = IconData(
    57900,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square kanban'.
  static const IconData squareKanban = IconData(
    57715,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square library'.
  static const IconData squareLibrary = IconData(
    58707,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square M'.
  static const IconData squareM = IconData(58631, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square menu'.
  static const IconData squareMenu = IconData(58455, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square minus'.
  static const IconData squareMinus = IconData(
    57716,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square mouse pointer'.
  static const IconData squareMousePointer = IconData(
    57857,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square parking off'.
  static const IconData squareParkingOff = IconData(
    58319,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square parking'.
  static const IconData squareParking = IconData(
    58318,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square pen'.
  static const IconData squarePen = IconData(57717, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square percent'.
  static const IconData squarePercent = IconData(
    58656,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square pi'.
  static const IconData squarePi = IconData(58508, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square pilcrow'.
  static const IconData squarePilcrow = IconData(
    58511,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square play'.
  static const IconData squarePlay = IconData(58501, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square plus'.
  static const IconData squarePlus = IconData(57718, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square power'.
  static const IconData squarePower = IconData(
    58709,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square radical'.
  static const IconData squareRadical = IconData(
    58823,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square round corner'.
  static const IconData squareRoundCorner = IconData(
    58956,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square scissors'.
  static const IconData squareScissors = IconData(
    58608,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square sigma'.
  static const IconData squareSigma = IconData(
    58509,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square slash'.
  static const IconData squareSlash = IconData(
    57719,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square split horizontal'.
  static const IconData squareSplitHorizontal = IconData(
    58297,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square split vertical'.
  static const IconData squareSplitVertical = IconData(
    58298,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square square'.
  static const IconData squareSquare = IconData(
    58898,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square stack'.
  static const IconData squareStack = IconData(
    58534,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square terminal'.
  static const IconData squareTerminal = IconData(
    57865,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square user round'.
  static const IconData squareUserRound = IconData(
    58474,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'square user'.
  static const IconData squareUser = IconData(58473, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square X'.
  static const IconData squareX = IconData(57720, fontFamily: 'LucideIcons');

  /// Lucide icon for 'square'.
  static const IconData square = IconData(57706, fontFamily: 'LucideIcons');

  /// Lucide icon for 'squircle'.
  static const IconData squircle = IconData(58750, fontFamily: 'LucideIcons');

  /// Lucide icon for 'squirrel'.
  static const IconData squirrel = IconData(58531, fontFamily: 'LucideIcons');

  /// Lucide icon for 'stamp'.
  static const IconData stamp = IconData(58302, fontFamily: 'LucideIcons');

  /// Lucide icon for 'star half'.
  static const IconData starHalf = IconData(57866, fontFamily: 'LucideIcons');

  /// Lucide icon for 'star off'.
  static const IconData starOff = IconData(58031, fontFamily: 'LucideIcons');

  /// Lucide icon for 'star'.
  static const IconData star = IconData(57721, fontFamily: 'LucideIcons');

  /// Lucide icon for 'step back'.
  static const IconData stepBack = IconData(58348, fontFamily: 'LucideIcons');

  /// Lucide icon for 'step forward'.
  static const IconData stepForward = IconData(
    58349,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'stethoscope'.
  static const IconData stethoscope = IconData(
    58096,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'sticker'.
  static const IconData sticker = IconData(58113, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sticky note'.
  static const IconData stickyNote = IconData(58114, fontFamily: 'LucideIcons');

  /// Lucide icon for 'store'.
  static const IconData store = IconData(58343, fontFamily: 'LucideIcons');

  /// Lucide icon for 'stretch horizontal'.
  static const IconData stretchHorizontal = IconData(
    57979,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'stretch vertical'.
  static const IconData stretchVertical = IconData(
    57980,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'strikethrough'.
  static const IconData strikethrough = IconData(
    57722,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'subscript'.
  static const IconData subscript = IconData(57947, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sun dim'.
  static const IconData sunDim = IconData(58008, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sun medium'.
  static const IconData sunMedium = IconData(58032, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sun moon'.
  static const IconData sunMoon = IconData(58033, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sun snow'.
  static const IconData sunSnow = IconData(58229, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sun'.
  static const IconData sun = IconData(57723, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sunrise'.
  static const IconData sunrise = IconData(57724, fontFamily: 'LucideIcons');

  /// Lucide icon for 'sunset'.
  static const IconData sunset = IconData(57725, fontFamily: 'LucideIcons');

  /// Lucide icon for 'superscript'.
  static const IconData superscript = IconData(
    57949,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'swatch book'.
  static const IconData swatchBook = IconData(58787, fontFamily: 'LucideIcons');

  /// Lucide icon for 'swiss franc'.
  static const IconData swissFranc = IconData(57726, fontFamily: 'LucideIcons');

  /// Lucide icon for 'switch camera'.
  static const IconData switchCamera = IconData(
    57727,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'sword'.
  static const IconData sword = IconData(58034, fontFamily: 'LucideIcons');

  /// Lucide icon for 'swords'.
  static const IconData swords = IconData(58035, fontFamily: 'LucideIcons');

  /// Lucide icon for 'syringe'.
  static const IconData syringe = IconData(58097, fontFamily: 'LucideIcons');

  /// Lucide icon for 'table 2'.
  static const IconData table2 = IconData(58104, fontFamily: 'LucideIcons');

  /// Lucide icon for 'table cells merge'.
  static const IconData tableCellsMerge = IconData(
    58827,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'table cells split'.
  static const IconData tableCellsSplit = IconData(
    58828,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'table columns split'.
  static const IconData tableColumnsSplit = IconData(
    58829,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'table of contents'.
  static const IconData tableOfContents = IconData(
    58914,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'table properties'.
  static const IconData tableProperties = IconData(
    58591,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'table rows split'.
  static const IconData tableRowsSplit = IconData(
    58830,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'table'.
  static const IconData table = IconData(57728, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tablet smartphone'.
  static const IconData tabletSmartphone = IconData(
    58638,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'tablet'.
  static const IconData tablet = IconData(57729, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tablets'.
  static const IconData tablets = IconData(58305, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tag'.
  static const IconData tag = IconData(57730, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tags'.
  static const IconData tags = IconData(58207, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tally 1'.
  static const IconData tally1 = IconData(58586, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tally 2'.
  static const IconData tally2 = IconData(58587, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tally 3'.
  static const IconData tally3 = IconData(58588, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tally 4'.
  static const IconData tally4 = IconData(58589, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tally 5'.
  static const IconData tally5 = IconData(58590, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tangent'.
  static const IconData tangent = IconData(58674, fontFamily: 'LucideIcons');

  /// Lucide icon for 'target'.
  static const IconData target = IconData(57731, fontFamily: 'LucideIcons');

  /// Lucide icon for 'telescope'.
  static const IconData telescope = IconData(58825, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tent tree'.
  static const IconData tentTree = IconData(58687, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tent'.
  static const IconData tent = IconData(57894, fontFamily: 'LucideIcons');

  /// Lucide icon for 'terminal'.
  static const IconData terminal = IconData(57732, fontFamily: 'LucideIcons');

  /// Lucide icon for 'test tube diagonal'.
  static const IconData testTubeDiagonal = IconData(
    58378,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'test tube'.
  static const IconData testTube = IconData(58377, fontFamily: 'LucideIcons');

  /// Lucide icon for 'test tubes'.
  static const IconData testTubes = IconData(58379, fontFamily: 'LucideIcons');

  /// Lucide icon for 'text cursor input'.
  static const IconData textCursorInput = IconData(
    57956,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'text cursor'.
  static const IconData textCursor = IconData(57955, fontFamily: 'LucideIcons');

  /// Lucide icon for 'text quote'.
  static const IconData textQuote = IconData(58530, fontFamily: 'LucideIcons');

  /// Lucide icon for 'text search'.
  static const IconData textSearch = IconData(58801, fontFamily: 'LucideIcons');

  /// Lucide icon for 'text select'.
  static const IconData textSelect = IconData(58337, fontFamily: 'LucideIcons');

  /// Lucide icon for 'text'.
  static const IconData text = IconData(58350, fontFamily: 'LucideIcons');

  /// Lucide icon for 'theater'.
  static const IconData theater = IconData(58662, fontFamily: 'LucideIcons');

  /// Lucide icon for 'thermometer snowflake'.
  static const IconData thermometerSnowflake = IconData(
    57734,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'thermometer sun'.
  static const IconData thermometerSun = IconData(
    57735,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'thermometer'.
  static const IconData thermometer = IconData(
    57733,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'thumbs down'.
  static const IconData thumbsDown = IconData(57736, fontFamily: 'LucideIcons');

  /// Lucide icon for 'thumbs up'.
  static const IconData thumbsUp = IconData(57737, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ticket check'.
  static const IconData ticketCheck = IconData(
    58802,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ticket minus'.
  static const IconData ticketMinus = IconData(
    58803,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ticket percent'.
  static const IconData ticketPercent = IconData(
    58804,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ticket plus'.
  static const IconData ticketPlus = IconData(58805, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ticket slash'.
  static const IconData ticketSlash = IconData(
    58806,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ticket X'.
  static const IconData ticketX = IconData(58807, fontFamily: 'LucideIcons');

  /// Lucide icon for 'ticket'.
  static const IconData ticket = IconData(57870, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tickets plane'.
  static const IconData ticketsPlane = IconData(
    58919,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'tickets'.
  static const IconData tickets = IconData(58918, fontFamily: 'LucideIcons');

  /// Lucide icon for 'timer off'.
  static const IconData timerOff = IconData(57928, fontFamily: 'LucideIcons');

  /// Lucide icon for 'timer reset'.
  static const IconData timerReset = IconData(57909, fontFamily: 'LucideIcons');

  /// Lucide icon for 'timer'.
  static const IconData timer = IconData(57823, fontFamily: 'LucideIcons');

  /// Lucide icon for 'toggle left'.
  static const IconData toggleLeft = IconData(57738, fontFamily: 'LucideIcons');

  /// Lucide icon for 'toggle right'.
  static const IconData toggleRight = IconData(
    57739,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'toilet'.
  static const IconData toilet = IconData(58937, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tornado'.
  static const IconData tornado = IconData(57879, fontFamily: 'LucideIcons');

  /// Lucide icon for 'torus'.
  static const IconData torus = IconData(58675, fontFamily: 'LucideIcons');

  /// Lucide icon for 'touchpad off'.
  static const IconData touchpadOff = IconData(
    58446,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'touchpad'.
  static const IconData touchpad = IconData(58445, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tower control'.
  static const IconData towerControl = IconData(
    58303,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'toy brick'.
  static const IconData toyBrick = IconData(58186, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tractor'.
  static const IconData tractor = IconData(58632, fontFamily: 'LucideIcons');

  /// Lucide icon for 'traffic cone'.
  static const IconData trafficCone = IconData(
    58633,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'train front tunnel'.
  static const IconData trainFrontTunnel = IconData(
    58635,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'train front'.
  static const IconData trainFront = IconData(58634, fontFamily: 'LucideIcons');

  /// Lucide icon for 'train track'.
  static const IconData trainTrack = IconData(58636, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tram front'.
  static const IconData tramFront = IconData(58024, fontFamily: 'LucideIcons');

  /// Lucide icon for 'transgender'.
  static const IconData transgender = IconData(
    58952,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'trash 2'.
  static const IconData trash2 = IconData(57741, fontFamily: 'LucideIcons');

  /// Lucide icon for 'trash'.
  static const IconData trash = IconData(57740, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tree deciduous'.
  static const IconData treeDeciduous = IconData(
    58098,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'tree palm'.
  static const IconData treePalm = IconData(57984, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tree pine'.
  static const IconData treePine = IconData(58099, fontFamily: 'LucideIcons');

  /// Lucide icon for 'trees'.
  static const IconData trees = IconData(58100, fontFamily: 'LucideIcons');

  /// Lucide icon for 'trello'.
  static const IconData trello = IconData(57742, fontFamily: 'LucideIcons');

  /// Lucide icon for 'trending down'.
  static const IconData trendingDown = IconData(
    57743,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'trending up down'.
  static const IconData trendingUpDown = IconData(
    58921,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'trending up'.
  static const IconData trendingUp = IconData(57744, fontFamily: 'LucideIcons');

  /// Lucide icon for 'triangle alert'.
  static const IconData triangleAlert = IconData(
    57746,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'triangle dashed'.
  static const IconData triangleDashed = IconData(
    58945,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'triangle right'.
  static const IconData triangleRight = IconData(
    58609,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'triangle'.
  static const IconData triangle = IconData(57745, fontFamily: 'LucideIcons');

  /// Lucide icon for 'trophy'.
  static const IconData trophy = IconData(58230, fontFamily: 'LucideIcons');

  /// Lucide icon for 'truck'.
  static const IconData truck = IconData(57747, fontFamily: 'LucideIcons');

  /// Lucide icon for 'turtle'.
  static const IconData turtle = IconData(58621, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tv minimal play'.
  static const IconData tvMinimalPlay = IconData(
    58864,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'tv minimal'.
  static const IconData tvMinimal = IconData(57858, fontFamily: 'LucideIcons');

  /// Lucide icon for 'tv'.
  static const IconData tv = IconData(57748, fontFamily: 'LucideIcons');

  /// Lucide icon for 'twitch'.
  static const IconData twitch = IconData(57749, fontFamily: 'LucideIcons');

  /// Lucide icon for 'twitter'.
  static const IconData twitter = IconData(57750, fontFamily: 'LucideIcons');

  /// Lucide icon for 'type outline'.
  static const IconData typeOutline = IconData(
    58886,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'type'.
  static const IconData type = IconData(57751, fontFamily: 'LucideIcons');

  /// Lucide icon for 'umbrella off'.
  static const IconData umbrellaOff = IconData(
    58695,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'umbrella'.
  static const IconData umbrella = IconData(57752, fontFamily: 'LucideIcons');

  /// Lucide icon for 'underline'.
  static const IconData underline = IconData(57753, fontFamily: 'LucideIcons');

  /// Lucide icon for 'undo 2'.
  static const IconData undo2 = IconData(58016, fontFamily: 'LucideIcons');

  /// Lucide icon for 'undo dot'.
  static const IconData undoDot = IconData(58453, fontFamily: 'LucideIcons');

  /// Lucide icon for 'undo'.
  static const IconData undo = IconData(57754, fontFamily: 'LucideIcons');

  /// Lucide icon for 'unfold horizontal'.
  static const IconData unfoldHorizontal = IconData(
    58433,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'unfold vertical'.
  static const IconData unfoldVertical = IconData(
    58434,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'ungroup'.
  static const IconData ungroup = IconData(58475, fontFamily: 'LucideIcons');

  /// Lucide icon for 'university'.
  static const IconData university = IconData(58344, fontFamily: 'LucideIcons');

  /// Lucide icon for 'unlink 2'.
  static const IconData unlink2 = IconData(57756, fontFamily: 'LucideIcons');

  /// Lucide icon for 'unlink'.
  static const IconData unlink = IconData(57755, fontFamily: 'LucideIcons');

  /// Lucide icon for 'unplug'.
  static const IconData unplug = IconData(58465, fontFamily: 'LucideIcons');

  /// Lucide icon for 'upload'.
  static const IconData upload = IconData(57757, fontFamily: 'LucideIcons');

  /// Lucide icon for 'usb'.
  static const IconData usb = IconData(58201, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user check'.
  static const IconData userCheck = IconData(57759, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user cog'.
  static const IconData userCog = IconData(58181, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user minus'.
  static const IconData userMinus = IconData(57760, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user pen'.
  static const IconData userPen = IconData(58880, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user plus'.
  static const IconData userPlus = IconData(57761, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user round check'.
  static const IconData userRoundCheck = IconData(
    58477,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'user round cog'.
  static const IconData userRoundCog = IconData(
    58478,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'user round minus'.
  static const IconData userRoundMinus = IconData(
    58479,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'user round pen'.
  static const IconData userRoundPen = IconData(
    58881,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'user round plus'.
  static const IconData userRoundPlus = IconData(
    58480,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'user round search'.
  static const IconData userRoundSearch = IconData(
    58748,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'user round X'.
  static const IconData userRoundX = IconData(58481, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user round'.
  static const IconData userRound = IconData(58476, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user search'.
  static const IconData userSearch = IconData(58749, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user X'.
  static const IconData userX = IconData(57762, fontFamily: 'LucideIcons');

  /// Lucide icon for 'user'.
  static const IconData user = IconData(57758, fontFamily: 'LucideIcons');

  /// Lucide icon for 'users round'.
  static const IconData usersRound = IconData(58482, fontFamily: 'LucideIcons');

  /// Lucide icon for 'users'.
  static const IconData users = IconData(57763, fontFamily: 'LucideIcons');

  /// Lucide icon for 'utensils crossed'.
  static const IconData utensilsCrossed = IconData(
    58102,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'utensils'.
  static const IconData utensils = IconData(58101, fontFamily: 'LucideIcons');

  /// Lucide icon for 'utility pole'.
  static const IconData utilityPole = IconData(
    58309,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'variable'.
  static const IconData variable = IconData(58487, fontFamily: 'LucideIcons');

  /// Lucide icon for 'vault'.
  static const IconData vault = IconData(58771, fontFamily: 'LucideIcons');

  /// Lucide icon for 'vegan'.
  static const IconData vegan = IconData(58272, fontFamily: 'LucideIcons');

  /// Lucide icon for 'venetian mask'.
  static const IconData venetianMask = IconData(
    58025,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'venus and mars'.
  static const IconData venusAndMars = IconData(
    58954,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'venus'.
  static const IconData venus = IconData(58953, fontFamily: 'LucideIcons');

  /// Lucide icon for 'vibrate off'.
  static const IconData vibrateOff = IconData(58012, fontFamily: 'LucideIcons');

  /// Lucide icon for 'vibrate'.
  static const IconData vibrate = IconData(57890, fontFamily: 'LucideIcons');

  /// Lucide icon for 'video off'.
  static const IconData videoOff = IconData(57765, fontFamily: 'LucideIcons');

  /// Lucide icon for 'video'.
  static const IconData video = IconData(57764, fontFamily: 'LucideIcons');

  /// Lucide icon for 'videotape'.
  static const IconData videotape = IconData(58575, fontFamily: 'LucideIcons');

  /// Lucide icon for 'view'.
  static const IconData view = IconData(57766, fontFamily: 'LucideIcons');

  /// Lucide icon for 'voicemail'.
  static const IconData voicemail = IconData(57767, fontFamily: 'LucideIcons');

  /// Lucide icon for 'volleyball'.
  static const IconData volleyball = IconData(58931, fontFamily: 'LucideIcons');

  /// Lucide icon for 'volume 1'.
  static const IconData volume1 = IconData(57769, fontFamily: 'LucideIcons');

  /// Lucide icon for 'volume 2'.
  static const IconData volume2 = IconData(57770, fontFamily: 'LucideIcons');

  /// Lucide icon for 'volume off'.
  static const IconData volumeOff = IconData(58922, fontFamily: 'LucideIcons');

  /// Lucide icon for 'volume X'.
  static const IconData volumeX = IconData(57771, fontFamily: 'LucideIcons');

  /// Lucide icon for 'volume'.
  static const IconData volume = IconData(57768, fontFamily: 'LucideIcons');

  /// Lucide icon for 'vote'.
  static const IconData vote = IconData(58288, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wallet cards'.
  static const IconData walletCards = IconData(
    58576,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'wallet minimal'.
  static const IconData walletMinimal = IconData(
    58577,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'wallet'.
  static const IconData wallet = IconData(57859, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wallpaper'.
  static const IconData wallpaper = IconData(58447, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wand sparkles'.
  static const IconData wandSparkles = IconData(
    58202,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'wand'.
  static const IconData wand = IconData(57925, fontFamily: 'LucideIcons');

  /// Lucide icon for 'warehouse'.
  static const IconData warehouse = IconData(58345, fontFamily: 'LucideIcons');

  /// Lucide icon for 'washing machine'.
  static const IconData washingMachine = IconData(
    58772,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'watch'.
  static const IconData watch = IconData(57772, fontFamily: 'LucideIcons');

  /// Lucide icon for 'waves ladder'.
  static const IconData wavesLadder = IconData(
    58943,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'waves'.
  static const IconData waves = IconData(57986, fontFamily: 'LucideIcons');

  /// Lucide icon for 'waypoints'.
  static const IconData waypoints = IconData(58694, fontFamily: 'LucideIcons');

  /// Lucide icon for 'webcam'.
  static const IconData webcam = IconData(57860, fontFamily: 'LucideIcons');

  /// Lucide icon for 'webhook off'.
  static const IconData webhookOff = IconData(58811, fontFamily: 'LucideIcons');

  /// Lucide icon for 'webhook'.
  static const IconData webhook = IconData(58231, fontFamily: 'LucideIcons');

  /// Lucide icon for 'weight'.
  static const IconData weight = IconData(58676, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wheat off'.
  static const IconData wheatOff = IconData(58274, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wheat'.
  static const IconData wheat = IconData(58273, fontFamily: 'LucideIcons');

  /// Lucide icon for 'whole word'.
  static const IconData wholeWord = IconData(58338, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wifi high'.
  static const IconData wifiHigh = IconData(58875, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wifi low'.
  static const IconData wifiLow = IconData(58876, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wifi off'.
  static const IconData wifiOff = IconData(57774, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wifi zero'.
  static const IconData wifiZero = IconData(58877, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wifi'.
  static const IconData wifi = IconData(57773, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wind arrow down'.
  static const IconData windArrowDown = IconData(
    58933,
    fontFamily: 'LucideIcons',
  );

  /// Lucide icon for 'wind'.
  static const IconData wind = IconData(57775, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wine off'.
  static const IconData wineOff = IconData(58275, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wine'.
  static const IconData wine = IconData(58103, fontFamily: 'LucideIcons');

  /// Lucide icon for 'workflow'.
  static const IconData workflow = IconData(58409, fontFamily: 'LucideIcons');

  /// Lucide icon for 'worm'.
  static const IconData worm = IconData(58846, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wrap text'.
  static const IconData wrapText = IconData(57927, fontFamily: 'LucideIcons');

  /// Lucide icon for 'wrench'.
  static const IconData wrench = IconData(57776, fontFamily: 'LucideIcons');

  /// Lucide icon for 'X'.
  static const IconData x = IconData(57777, fontFamily: 'LucideIcons');

  /// Lucide icon for 'youtube'.
  static const IconData youtube = IconData(57778, fontFamily: 'LucideIcons');

  /// Lucide icon for 'zap off'.
  static const IconData zapOff = IconData(57780, fontFamily: 'LucideIcons');

  /// Lucide icon for 'zap'.
  static const IconData zap = IconData(57779, fontFamily: 'LucideIcons');

  /// Lucide icon for 'zoom in'.
  static const IconData zoomIn = IconData(57781, fontFamily: 'LucideIcons');

  /// Lucide icon for 'zoom out'.
  static const IconData zoomOut = IconData(57782, fontFamily: 'LucideIcons');
}
